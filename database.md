...
| `consideration_logs` | K2 | **MSSQL** `CompensateHistory` (PK `ActionID`) | id | `doc_no` → compensation_documents | ประวัติพิจารณาทุกขั้น (ผู้พิจารณา · Section · ผล · เวลา) · `result_category` (APPROVE/REJECT/**CANCELLED**/PENDING) สำหรับ filter **ประกันรายได้ / ไม่ประกันรายได้ / ยกเลิกโดยระบบ / ยังไม่มีผล** หน้ารายงานตรวจสอบประกันรายได้ (k2-report · SDD v7.5) · **✅ DP-7 ปิดแล้ว 2026-08-24 — เป็นตารางของ SBPGI เอง (timeline เต็ม ผูก `transaction_id` ของ engine)** ไม่ต่อยอดบน `sps_store.workflow_history` ของ engine (engine เก็บ timeline แต่ไม่มีรหัสผลพิจารณา/ไฟล์แนบ) — กระทบ `GET /sbpgi/document/{docNo}/timeline` โดยตรง · [`SBP/SBPGI-vs-existing-system.md`](SBP/SBPGI-vs-existing-system.md) §4 |
...
|---|---|---|
| `workflow_instances` · `workflow_tasks` | **`@srm/glb-workflow`** ใน schema **`sps_store`** — `workflow_transaction` (instance · 19,283 แถว) · `workflow_approver` (prepared approvers · 96,542 แถว) · `workflow_history` (timeline · 38,010 แถว) | SBPGI ขอ **workflow version ใหม่** 1 ตัว แล้วเรียก `initializeWorkflow` → `addPreApprover` → `eventWorkflow` · **✅ ชื่อ function ยืนยันแล้ว 2026-08-14** (API 8 ตัวตามชีต `Detail` ของ LLDD ฝั่ง lib) · **✅ DP-1 ปิดแล้ว 2026-08-17** — `referenceId` = `compensation_documents.id` (surrogate · ส่งเป็น string) |
...
> 🔴 **ค่าของ SBPGI ใน `common_code` / `mas_param` ยังไม่มีอยู่จริง — ต้อง seed เอง (ระบุชัด 2026-08-25):** `SBPGI_APPROVE_LIMIT` · `SBPGI_DECISION` · `SBPGI_DATASOURCE` และค่าใน `mas_param` (`SBPGI_IMPACT_RADIUS_*` · `SBPGI_SALES_DAYS_MIN` · `SBPGI_GROWTH_RATE_MAX`) เป็นแถวที่ **การออกแบบวางแผนจะเพิ่ม** ไม่ใช่ของเดิมที่ reuse ได้ทันที — งาน seed อยู่ใน `LLDD-BE-Data-Migration-Cutover` และมี SQL ตัวอย่างใน `LLDD-BE-Integration-SBP-Platform` 5.5.2
>
> 🔴 **กับดัก schema:** `mas_param` มี**เฉพาะ `sps_store`** (`sps_auth` ไม่มีตารางนี้เลย) · `common_code` มี**ทั้งสอง schema แต่คนละตาราง** — `sps_store` 14 คอลัมน์ 2,609 แถว vs `sps_auth` **13 คอลัมน์ 2,594 แถว** (ชุดเก่าของ auth-backend) · **SBPGI ใช้ `sps_store` เท่านั้น** และต้องเขียน schema นำหน้าใน SQL ทุกครั้ง (กับดักเดียวกับตาราง `workflow_*`)
...
ดูรายละเอียดในแถว `fcs_qssi_score` ของ Data Dictionary โซน A ด้านบน · สรุป: ชื่อ **เอกพจน์** · reuse ของเดิม · มี `POST /performance/import-qssi` + staging `fcs_tmp_qssi_score` ทำงานอยู่แล้ว · **✅ DP-4 ปิดแล้ว 2026-08-24 — ไม่แก้ตารางเดิม (NOT NULL / UNIQUE / index) เพราะ SBPGI อ่านอย่างเดียว**
...
| # | เรื่อง | สถานะ |
|---|---|---|
| DP-1 … DP-12 | **12 ข้อ · ปิดแล้ว 8 (DP-1 · DP-3 · DP-9 เมื่อ 2026-08-10 · DP-5 · DP-10 · DP-4 · DP-7 · DP-8 ปิด 2026-08-24) เหลือค้าง 4** — DP-2 (ไม่มี PK/index บน `sps_store.workflow_transaction`) · DP-6 (`interface_transactions` ออกแบบใหม่หรือลอก `statement_summary`) · DP-11 (ตัวเลขเงินประกันรายได้เก็บที่ไหน) · DP-12 (audit ของ master เอากลับมาหรือไม่) | **ยังไม่ตัดสิน** — รายละเอียดครบทุกข้อพร้อมข้อดี/ข้อเสียอยู่ที่ [`SBP/SBPGI-vs-existing-system.md`](SBP/SBPGI-vs-existing-system.md) §4 · **เอกสารนี้บันทึกไว้เฉย ๆ ไม่เปลี่ยนดีไซน์ตามข้อเสนอ** |
