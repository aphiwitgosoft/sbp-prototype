# Database — FGI/FCS + K2 (Target Schema ระบบใหม่ SGI)
...
ระบบใหม่ **รวม EAI และ K2 เข้าเป็นส่วนหนึ่งของ SGI** — งาน FGI/FCS batch และงานเอกสาร/workflow K2 ทำงานบน **ฐานข้อมูลเดียวกัน** ไม่มีการส่งไฟล์ผ่าน EAI อีกต่อไป (ดูรายละเอียด flow ที่ [workflow.md](workflow.md))

ผลต่อ schema:
- ไฟล์ภายใน `BPM06001O_` (48 ฟิลด์) / `BPM06002O_` / `BPM06003O_` ที่เคยส่งผ่าน EAI ไป K2 → แทนด้วย FK `sgi_compensation_documents.impact_process_id` เชื่อมตรงในฐานข้อมูลเดียวกัน
...
- ตาราง tracking เดิม `FGI_CONFIRM_RECEIVE_DATA` → แทนด้วย `sgi_interface_transactions` (typed FK)
...
- **21 ตาราง** ใน Target Schema เดียว (1 schema ใช้ร่วมกัน) — 34 ตารางเดิม **ตัดออก 10 ตารางที่ระบบ SBP ปัจจุบันมีอยู่แล้ว** เมื่อ 2026-08-06 (workflow engine · store/zone/employee master · email template · config — ดูหัวข้อ "ตารางที่ตัดออกรอบ 2" ท้ายไฟล์) แล้ว **ตัดอีก 2 ตาราง** (`job_configs` · `job_run_histories`) เมื่อ 2026-08-06 เพราะตัด 2 tab ควบคุมของหน้า Batch Job (ดูหมายเหตุในโซน C) และ **ตัด `audit_logs`** เมื่อ 2026-08-07 เพราะยกเลิกระบบ audit ของ master · แล้ว **มติ 2026-09-13** เพิ่ม `sgi_fgi_new_store_compensations` — ค่าชดเชยฝั่ง**ร้านเปิดใหม่ รายงวด** (ORA `FGI_NEW_STORE_COMPENSATE` 23,628 แถวจริง) · `sgi_fgi_impact_stores` เป็น 1 แถวต่อคู่ร้าน **ไม่มีมิติงวด** จึงเก็บแทนไม่ได้ — งวดใหม่จะทับงวดเก่า
...
- **ตัดสินใจ 2026-08-05:** RBAC (`roles`/`menus`/`menu_permissions`/`user_accounts`) และผู้ปฏิบัติงาน (`operator_assignments`) **ไม่สร้างใน SGI** — ใช้ระบบสิทธิ์/ผู้ใช้ของระบบ SBP เดิม (ดูหัวข้อ "ตารางที่ตัดออก" ท้าย Zone C) · การตัดนี้ทำให้เหลือ 29 ตาราง ก่อนเพิ่มอีก 5 ตารางจากการเทียบ DB เดิมของ K2 (2026-08-06) → รวมเป็น 34
- **ตรวจกับฐานข้อมูลจริงแล้ว 07/08/2026** — ยืนยันว่าโซน A และแกนเอกสารโซน B ไม่มีของเดิมให้ reuse · ดูหัวข้อ [ผลการเทียบกับฐานข้อมูลจริง (07/08/2026)](#ผลการเทียบกับฐานข้อมูลจริง-07082026) และ **ข้อค้างตัดสินใจ 12 ข้อ (DP-1 … DP-12) — ปิดแล้ว 8 เหลือค้าง 4** ใน [`SBP/SBPGI-vs-existing-system.md`](SBP/SBPGI-vs-existing-system.md) §4
...
> **⚠️ Invariant ของ `doc_no` (บันทึก 2026-08-17):** `sgi_compensation_documents.doc_no` เป็น **nullable UNIQUE** (มติ DP-1 ให้ PK เป็น `id`) แต่ **ตารางลูก 6 ตัว FK ไป `doc_no` แบบ `NOT NULL`** (`sgi_document_new_stores` · `sgi_document_external_factors` · `sgi_document_competitors` · `sgi_document_cost_details` · `sgi_document_attachments` · `sgi_consideration_logs`)
>
> ⇒ **ต้องออกเลขเอกสารใน INSERT เดียวกับที่สร้างแถวเสมอ** — Job 8 ทำแบบนี้อยู่แล้ว (`INSERT … (doc_no, year, running_no, impact_process_id, …)`) · ถ้าปล่อย `doc_no` ว่างไว้ เอกสารจะเปิด workflow ได้แต่**บันทึกรายละเอียดอะไรไม่ได้เลย** · ถ้าภายหลังธุรกิจต้องการ "เปิดเรื่องก่อนออกเลข" จริง ต้องเปลี่ยน FK ของลูกทั้ง 6 ไปที่ `sgi_compensation_documents(id)` ก่อน

| 2 | B | `sgi_compensation_documents.id` | **surrogate PK ของเอกสาร (มติ DP-1 = B)** — ค่าที่ส่งเป็น `reference_id` ให้ workflow engine · `doc_no` (`YYYY/xxxxx` ค.ศ.) เป็น **business key แบบ UNIQUE** ที่ผู้ใช้เห็น ไม่ใช่คีย์ที่ engine ยึด |
| 3 | B | `transaction_id` | Workflow instance หนึ่งชุดต่อเอกสาร — **อยู่ที่ `sps_store.workflow_transaction` ของ engine กลาง ไม่ใช่ตารางของ SGI** (เดิมเรียก `instance_id`) |
...
| 5 | C | `employee_id` | ผู้ปฏิบัติงานที่อ้างร่วมกันทุกขั้น — ตัวตน/สิทธิ์เมนูมาจากระบบ SBP เดิม (auth-backend) ผ่าน user-context header ไม่ใช่ตารางใน SGI |

> ### ✅ มติ 2026-08-21 — DP-10 ปิดแล้ว: SGI อยู่ใน `srm-sps-spsap-store-backend` เดิม
>
> **ไม่แยก backend ใหม่** — SGI เป็นโมดูลใน `SBP/srm-sps-spsap-store-backend` (NestJS + TypeORM) ร่วมกับระบบ SBP เดิม
> ผลที่ตามมา: ใช้ connection/schema `sps_store` เดียวกัน เรียก `@srm/glb-workflow` และ email-lib ได้ตรง ๆ ไม่ต้องข้าม service · deploy/CI ไปตาม pipeline ของ store-backend · ตาราง 20 ตัวของ SGI สร้างใน schema เดียวกับของเดิม (ชื่อไม่ชนกัน — ตรวจแล้วด้วย `tools/check_docs.py`)
...
- ลำดับ import ที่ปลอดภัย: master โซน C → โซน A (`sgi_fgi_impact_processes` ก่อน แล้วลูกทั้งหมด) → โซน B (`sgi_compensation_documents` ก่อน แล้ว `document_*`) → **ท้ายสุดคือการ initialize workflow ผ่าน engine กลาง** (`sps_store.workflow_*` — ไม่ใช่ตารางของ SGI แล้ว)
...
| `sgi_fgi_impact_stores` | FGI/FCS | **ORA** `FGI_IMPACT_STORE` (PK `IMPACT_STORE_ID` · business key `STORECODE_I`+`MONTH`+`YEAR`) | id | `impact_process_id` → sgi_fgi_impact_processes · `impacted_store_code` → sgi_impacted_stores | คู่ร้านกระทบ–เปิดใหม่ · **`verify_status` (W/P/N — ผลกฎ DENY/ON_PROCESS ของ Job 2)** แยกจาก **`sales_request_status` (W/P/Y/E — *การขอ* ยอดขาย IAS ของ Job 4/5) · ผลทางธุรกิจอยู่ที่ `sgi_fgi_impact_sales_summaries.sales_status` (W/P/Y/N/E)** · `created_by`/`updated_by` (ALM/STA/USER) · ข้อมูล %/ยอดชดเชยต่อคู่ร้าน <br><small>เพิ่ม verify_status + created_by/updated_by/created_at เมื่อ 2026-09-02 (ปิดช่องว่าง G1/G2 ของ LLDD Job 2)</small> |
...
| `sgi_fgi_impact_competitors` | FGI/FCS | **ORA** `FGI_IMPACT_COMPETITOR` (มีคอลัมน์ `IMPACT_COMPETITOR_ID` จาก sequence แต่ **DDL ไม่ได้ประกาศ PRIMARY KEY หรือ UNIQUE เลย** · มีแต่ index ธรรมดา `IDX_FGI_IMPACT_COMPETITOR(STORECODE_I, MONTH, YEAR)`) | id | `impact_process_id` → sgi_fgi_impact_processes · → sgi_document_competitors (นำเข้า) | คู่แข่งจาก ALLMAP (data_source=ALM) · งวดล่าสุดต่อร้าน |
| `fcs_qssi_score` | FGI/FCS | **ORA** `FCS_QSSI_SCORE` (`STORE_ID`+`CATEGORY`+`MONTH`+`YEAR`) — ⚠ **ไม่ต้อง migrate ใหม่ ของเดิมมีข้อมูลครบแล้ว · อ่านอย่างเดียว (READ-ONLY)** — ระบบ SBP เดิมเป็นคนนำเข้าผ่าน `POST /performance/import-qssi` · **SGI ตัด Job 1 ImportQSSI ออกจากขอบเขตแล้ว (2026-08-24)** จึงไม่มีการเขียนตารางนี้จากฝั่งเรา (ปิด DP-4) · **หมวดที่ SGI ใช้ 6 ค่า: `8` = Result · `9` = Process · `12` = สินค้าขาด · `1` = บริการ · `10` = Follow up · `16` = สินค้าหมดอายุ** (ลำดับนี้คือลำดับฟิลด์ 9–14 ของ `FRBC0001` · ถอดชื่อได้ 2026-09-16 จากรายงาน `RT040035` ของ STA เทียบกับไฟล์จริง — ดู `docs/IAS-STA-interface-files.md`) | id | UK: store_id + category_code + งวด (**ยังไม่มีจริงในของเดิม** — ดู DP-4) | คะแนน QSSI 6 หมวด (8,9,12,1,10,16) — **มีอยู่จริงใน `sps_store.fcs_qssi_score` (ชื่อ *เอกพจน์*) 24,284,545 แถว · 7 คอลัมน์** (`SBP/db-schema-sps_store.md`) และมี **import pipeline ที่ทำงานอยู่แล้ว**: `POST /performance/import-qssi` + staging `fcs_tmp_qssi_score` + `performance.service.ts` → **ห้ามสร้างตาราง/entity ใหม่ ให้ reuse ของเดิม** · ชื่อพหูพจน์ `fcs_qssi_scores` **ผิด ห้ามใช้** · ข้อจำกัดที่ต้องปิด: 4 คอลัมน์คีย์ (`store_id`/`category`/`month`/`year`) เป็น **nullable** · index มีแค่ `fcs_qssi_score_pkey` บน `id` → **✅ DP-4 ปิดแล้ว 2026-08-24 — อ่านอย่างเดียว ไม่แก้ constraint/index ของตารางเดิม จึงไม่ต้อง sign-off เจ้าของ `performance.service.ts`** · `fcs_qssi_score_bak_20260710` (18,577,924 แถว) เป็น snapshot **ห้ามอ่าน/ห้าม join** |
| `sgi_interface_transactions` | ใหม่ | **ORA** `FGI_CONFIRM_RECEIVE_DATA` — ⚠ `TRANSACTION_PK` เป็น polymorphic ต้องแตกตาม `DATA_NAME` เป็น typed FK ตอน migrate | id | typed FK: `impact_process_id` / `sales_summary_id` / `doc_no` | แทน FGI_CONFIRM_RECEIVE_DATA — เลิก polymorphic PK + purge ทำงานจริง · **คอลัมน์ `last_ack_notified_on` (DATE · ใหม่ 2026-08-07)** — marker กันงาน watchdog ส่งอีเมลเตือนซ้ำในวันเดียวกัน (ย้ายมาจาก `audit_logs` ที่ถูกยกเลิก) (แก้ E20) · **ยังไม่ตัดสิน (DP-6)** ว่าจะออกแบบใหม่ หรือลอกแพตเทิร์น `statement_summary` ของระบบเดิม — [`SBP/SBPGI-vs-existing-system.md`](SBP/SBPGI-vs-existing-system.md) §4 |

### Zone B · K2 — เอกสารประกันรายได้และ Workflow ภายใน

| ตาราง | ที่มา | ตารางต้นทาง (Migration) | PK | FK / ความสัมพันธ์หลัก | บทบาท |
|---|---|---|---|---|---|
| `sgi_compensation_documents` | K2 | **MSSQL** `CompensateFlow` (PK `CompDocumentID` → `doc_no`) + join **ORA** `FGI_IMPACT_STORE_INFO` เพื่อเติม `impact_process_id` | **`id` (surrogate · มติ DP-1 = ทางเลือก B 2026-08-10)** · `doc_no` เป็น **UNIQUE** ไม่ใช่ PK | `status_code` · `current_section_code` · `impacted_store_code` · **`impact_process_id` (ใหม่)** | เอกสารประกันรายได้ — หัวใจโซน B · FK ใหม่เชื่อม hub โซน A แทนไฟล์ 48 ฟิลด์ · **คอลัมน์ที่เติมจาก CompensateFlow เดิม (2026-08-06):** `round_no`/`loop_no` (= CompMainLoopNo/CompLoopNo — หน้าจอแสดง "รอบ 1 · ครั้งที่ 3") · `allmap_url` (= CompUrlMap — ปุ่ม Link To ALLMAP) · **`statement_id`** (= CompStatementID — โยงกลับ SBP Statement ที่เป็นต้นทางการสร้างเอกสารตามกระบวนการ FS ใหม่) · **`statement_date` (ใหม่ 2026-08-06)** — วันที่ของ SBP Statement เก็บเป็น **ค.ศ.** เพราะรายงานตรวจสอบประกันรายได้ตาม **SDD สไลด์ 60** ใช้ตัวกรอง Period Statement เป็นช่วง **วัน/เดือน/ปี (ค.ศ.)** และแสดงเป็นคอลัมน์ "Period Statement" ในผลลัพธ์ 14 คอลัมน์ · `account_year`/`account_month` (งวดบัญชี) · `approver_snapshot` (JSONB — FC/Section/Manager/GM/AVP + ชื่อ/อีเมล ณ เวลาเปิดเอกสาร ตามที่ CompensateFlow เก็บไว้ 25 คอลัมน์: **จำเป็นเป็นพิเศษเมื่อ RBAC ย้ายไปใช้ระบบเดิม เพราะตำแหน่งจาก HR Connect เปลี่ยนได้ และผู้รักษาการเป็นผู้อนุมัติไม่ได้**) |
| `sgi_document_new_stores` | K2 | **MSSQL** `ImpactProfile` (ฝั่ง `_N`) + **ORA** `FGI_NEW_STORE_INFO` / `FGI_NEW_STORE_COMPENSATE` (%ชดเชย + ยอดต่อร้าน) | id | `doc_no` → sgi_compensation_documents | ร้านเปิดใหม่ · `distance_km` · `compensate_percent` (**ผลรวมต้อง = 100%**) · `compensate_amount` (ใหม่ — ยอดชดเชยร้านถูกกระทบ × %ชดเชย คำนวณ/ปัดเศษที่ BE · ผลรวมทุกแถวต้องเท่ากับยอดชดเชยของเอกสารพอดี · แสดงในคอลัมน์ "เงินชดเชย (ร้านใหม่)" ของตารางร้านเปิดใหม่ — **กราฟสัดส่วนเงินชดเชยถูกถอดออก 2026-08-06**) |
| `sgi_document_competitors` | K2 | **MSSQL** `CompetInCompenProfile` (+ ไฟล์ `BPM06003O_` 14 ฟิลด์) | id | `doc_no` · `competitor_code` → sgi_competitors | คู่แข่งในเอกสาร **ระดับสาขา** · `source_system` = ALLMAP (จาก pipeline) / USER (ผู้ใช้เพิ่มเอง) · **คอลัมน์ที่ยืนยันจากไฟล์จริง (2026-08-06):** `competitor_code` เป็นรหัสจาก **ALLMAP** แบบตัวเลข/ตัวอักษรผสม (`4832`, `TD58_08`, `LS3550`) — **ไม่ใช่** รหัสแบรนด์ 01–11 · `branch_name` (ชื่อสาขาคู่แข่ง เช่น "ตลาดศรีวานิช") · `zone_code` + `subzone_code` (01–07) · `open_date`/`close_date` ของคู่แข่ง (ดู `docs/K2-interface-files.md`) |
| `sgi_document_external_factors` | K2 | **MSSQL** `FactorInCompenProfile` | id | `doc_no` · `factor_code` → sgi_external_factors | ปัจจัยภายนอกที่ใช้ในเอกสาร + ช่วงวันที่ |
| `sgi_consideration_logs` | K2 | **MSSQL** `CompensateHistory` (PK `ActionID`) | id | `doc_no` → sgi_compensation_documents | ประวัติพิจารณาทุกขั้น (ผู้พิจารณา · Section · ผล · เวลา) · `result_category` (APPROVE/REJECT/**CANCELLED**/PENDING) สำหรับ filter **ประกันรายได้ / ไม่ประกันรายได้ / ยกเลิกโดยระบบ / ยังไม่มีผล** หน้ารายงานตรวจสอบประกันรายได้ (k2-report · SDD v7.5) · **✅ DP-7 ปิดแล้ว 2026-08-24 — เป็นตารางของ SGI เอง (timeline เต็ม ผูก `transaction_id` ของ engine)** ไม่ต่อยอดบน `sps_store.workflow_history` ของ engine (engine เก็บ timeline แต่ไม่มีรหัสผลพิจารณา/ไฟล์แนบ) — กระทบ `GET /sgi/document/{docNo}/timeline` โดยตรง · [`SBP/SBPGI-vs-existing-system.md`](SBP/SBPGI-vs-existing-system.md) §4 |
| `sgi_document_attachments` | K2 | **MSSQL** `CompDocAttachment` + `CompTempAttachment` (+ `AttachFileProfile` สำหรับสถานะอัปโหลด/purge) | id | `doc_no` → sgi_compensation_documents | ไฟล์แนบ ≤ 5MB ต่อไฟล์ · แยกตาม Section ที่แนบ · **เติมจาก AttachFileProfile เดิม (2026-08-06):** `file_size` · `upload_status` + `upload_message` (ผลอัปโหลดขึ้น object storage) · `purge_flag`/`storage_delete_status` (lifecycle ลบไฟล์บน S3 — ของเดิมมี FlagPurgeData/FlagDeleteS3/StatusCodeDeleteS3 ครบ) |
| `sgi_compensation_histories` | K2 | **ORA** `FGI_IMPACT_STORE_COMPENSATE` + **MSSQL** `CompensateFlow` (แถวรอบก่อนหน้าของร้านเดียวกัน) | id | `store_code` · `ref_doc_no` | ประวัติชดเชยต่อร้าน/รอบ · `submit_account_month` เดือนส่งบัญชี (→ ไฟล์ FRBC0001 ของ Job 6) · ⚠ **ต้องตัดสิน DP-11 ก่อนสร้างตารางนี้** — SGI เป็นต้นทางตัวเลขเงินประกันรายได้ หรือ `fr_store_insure` ยังคีย์มือ (ข้อธุรกิจล้วน) · [`SBP/SBPGI-vs-existing-system.md`](SBP/SBPGI-vs-existing-system.md) §4 |
| `sgi_document_cost_details` ★ | K2 (ImpactCostDetail) | **MSSQL** `ImpactCostDetail` (PK `ImpCostID`) | id | `doc_no` → sgi_compensation_documents · `new_store_code` | **(เพิ่ม 2026-08-06)** ยอดชดเชย**แยกรายเดือน/รายร้านเปิดใหม่** — `cost_year`/`cost_month` · `cost_target` (เป้ายอดขาย) · `cost_amount` · แยกค่าของร้านใหม่ (`_n`) และร้านใหม่สะสม (`_nc`) ตาม ImpactCostDetail เดิม · ของเดิมในโครงเรามีแค่ยอดรวมต่อเอกสาร + %ต่อร้าน ทำให้ทวนยอดรายเดือนกับ Statement/SAP ไม่ได้ |
| `sgi_document_running_numbers` ★ | K2 (RunningNumber) | **MSSQL** `RunningNumber` | `year` | ออกเลขให้ sgi_compensation_documents | **(เพิ่ม 2026-08-06)** ตัวนับเลขเอกสารต่อปี **ค.ศ.** (`last_running_no`) — ปีเป็น ค.ศ. ตามมติ 2026-08-06 (ดู `api.md`) **ห้ามเก็บ พ.ศ.** — ออกเลข `YYYY/xxxxx` แบบ atomic (`UPDATE … RETURNING` / row lock) กันเลขชนกันเมื่อ batch และผู้ใช้สร้างพร้อมกัน · เดิมโครงเราไม่ระบุที่เก็บตัวนับ |

### Zone C · Shared — Master ที่ SGI เป็นเจ้าของ

(RBAC/ผู้ใช้/config/email template/workflow → ใช้ของระบบ SBP เดิม · audit ยกเลิก 2026-08-07)

| ตาราง | ที่มา | ตารางต้นทาง (Migration) | PK | FK / ความสัมพันธ์หลัก | บทบาท |
|---|---|---|---|---|---|
| `sgi_impacted_stores` | K2 | **ORA** `FGI_IMPACT_STORE` (ฝั่ง `_I` · distinct) + **MSSQL** `CompensateFlow.CompTransferSBPDate` | `store_code` | = `impacted_store_code` ของโซน A (สะพานหลักสองระบบ) · subset SP ของ `stores` | ข้อมูลร้าน SP master · **`transfer_sbp_date` (เพิ่ม 2026-08-06 = CompTransferSBPDate เดิม)** — วันที่โอนเป็นร้าน SP ใช้กับเงื่อนไขร้านก่อน/หลัง 1/10/2014 ของ Approve Flow เดิม · **มติ DP-3 (2026-08-10) = ตาราง snapshot บางส่วน** — เก็บ**เฉพาะร้านที่เคยเข้ารอบชดเชยจริง** (ไม่ sync ทั้ง master 11,583 แถว) เติมทีละแถวแบบ upsert ตอน pipeline สร้าง `sgi_fgi_impact_processes` · เหตุผล: `v_fr_store_active` ตัดร้านที่ยกเลิกเกิน 1 เดือนออก → ถ้าใช้ view เอกสารย้อนหลังจะกลายเป็น "ไม่พบร้าน" · snapshot ของ SGI — กระทบทั้ง DDL และขอบเขต migration · [`SBP/SBPGI-vs-existing-system.md`](SBP/SBPGI-vs-existing-system.md) §4 |
...
>   emailId,     // เลข template — SGI อ่านจาก sps_store.workflow_route.email_id ของ route ที่เพิ่งเดิน
...
>
> **สิ่งที่ปลดล็อกได้จากมติใหม่:** อีเมลเตือนงานค้างรายสัปดาห์ + escalation 30/45/60 วัน **ไม่ใช่ transition** จึงไม่มี route ให้แขวน `email_id` — เดิมเป็นรูโหว่ของ DP-5 · ตอนนี้ SGI ส่งเองได้ โดยเก็บเลข template ของเมลกลุ่มนี้ไว้ที่ **`mas_param`** (ไม่ hardcode)
>
...
> 2. `email_template` จริงใน `sps_store` มี **12 คอลัมน์** (`email_template_id` · `email_template_name` · `email_template_desc` · `subject_format` · `body_format` · `sender` · `email_from` · `active_flag` · `create_by/date` · `update_by/date`) — **ชื่อในชีต Database ของเอกสาร lib (`email_id`/`email_name`/`subject_mail`/`body_mail`/`mail_from`/`mail_from_name`) เป็นชื่อที่เสนอไว้ ไม่ตรง production** · seed template ของ SGI ต้องใช้ชื่อจริง
...
> 5. 🔴 **ต้องยืนยันกับทีมเจ้าของ `@srm/glb-workflow`:** ถ้า engine ส่งเมลเองด้วยบน route ที่มี `email_id` ผู้อนุมัติจะได้ **เมลซ้ำ 2 ฉบับ** — ทางแก้คือใช้ `workflow_route.email_id` เป็น *ค่าอ่านอย่างเดียว* ให้ SGI ไปเรียก lib เอง และให้ฝั่ง engine ปิดการส่ง

### Seed data ที่ต้องใส่ลงตารางของระบบ SBP เดิม (มติ 2026-08-17)

SGI **ไม่สร้างตารางใหม่** สำหรับ 3 ชุดนี้ แต่ต้อง **INSERT แถวของตัวเอง** ลงตารางของระบบเดิม — **ทีม SGI เป็นผู้ทำเอง**

| ชุด | ตารางปลายทาง (`sps_store`) | ผู้รับผิดชอบ | ขอบเขตที่อนุญาต |
|---|---|---|---|
| **Email template 8 ฉบับ** (EM-01…EM-08) | `email_template` (ของทีมอื่น 126 แถว) | **Butsaba \<But\> Podamrong** | `INSERT` แถวใหม่ + `active_flag='Y'` เท่านั้น · ห้าม `UPDATE`/`DELETE` แถวของทีมอื่น · ต้องใช้ชื่อคอลัมน์จริง (`email_template_name` · `subject_format` · `body_format` · `sender` · `email_from`) · **ตัวแปรในข้อความเป็น `${ชื่อ}` ไม่ใช่ `{ชื่อ}`** · `sender` = อีเมลผู้ส่ง · `email_from` = ชื่อที่แสดง (สลับกับที่ชื่อคอลัมน์ชวนให้เข้าใจ) · ชุดประกันรายได้เดิม (id 1501010–1501044) ใช้ `noreply@cpall.co.th` / `SBP Mall System` ทั้ง 33 แถว |
| **Workflow version เริ่มต้น 1 ชุด** | **10 ตารางนิยาม** ของ engine (จาก 13 ตาราง — อีก 3 ตัว `workflow_transaction`/`workflow_history`/`workflow_approver` เป็น runtime ที่ lib เขียนเอง ห้ามแตะ): (`workflow` · `workflow_version` · `workflow_state` · `workflow_status` · `workflow_event` · `workflow_route` · `workflow_group` · `workflow_group_map` · `workflow_part` · `workflow_part_display`) | **Aphiwit \<Bank\> Khammoon** | `INSERT` **version ใหม่หมายเลขเดียว** ของ SGI · 🔴 ห้ามแตะ version ของระบบอื่นเด็ดขาด (`workflow_transaction` 19,327 แถวใช้ร่วมกันทั้งองค์กร) · `workflow_route.email_id` ผูกเลข template จากชุดบน |
...
SGI เป็น backend ใหม่ที่จะเสียบเข้าสถาปัตยกรรม SBP ปัจจุบัน (FE → BFF → backend ต่อโดเมน) ซึ่งมีระบบผู้ใช้/สิทธิ์อยู่แล้ว จึง**ไม่สร้างตาราง RBAC/ผู้ปฏิบัติงานของตัวเอง** — หน้าจอ `k2-permissions.html` (SRS 3.1.1) และ `k2-operators.html` (SRS 3.1.8) ถูกถอดออกจาก sidebar ของ prototype (ไฟล์ยังเก็บไว้อ้างอิง):
...
| `user_accounts` | AWS Cognito + auth-backend `users`/`user_group_memberships` — SGI รับตัวตนจาก BFF ผ่าน header `x-user-id`/`x-user-group-id` ไม่เก็บบัญชีเอง |
...
ผลที่ตาม: 8 role (00–10) ของ SRS ถูก map เป็น group ใน auth-backend · การแก้สิทธิ์/กลุ่มลง audit ของระบบเดิม ไม่ใช่ `audit_logs` ของ SGI

> ⚠️ **แก้บันทึกเดิมที่ผิด (2026-08-20):** เอกสารรุ่นก่อนเขียนว่า *"`workflow_tasks.assignee_employee_id` ยังอยู่"* — **ไม่จริง** ตาราง `workflow_tasks` ถูกตัดออกจากโครง 20 ตารางไปตั้งแต่รอบ reuse 2026-08-06 (ย้ายไป engine 13 ตารางใน `sps_store`) จึง **ไม่มีคอลัมน์เก็บ "เจ้าของงาน" ในตารางของ SGI เลย**
>
...
> - **"คนเดิม"** ได้จากการ query `sgi_consideration_logs` ของ **เอกสารรอบก่อนหน้าของร้านเดียวกัน** — แถวล่าสุดที่ `section_code` ตรงกับขั้นที่จะมอบหมาย → **`consider_by`**
...
| `DecisionProfile` | **`common_code`** (`code_type = SGI_DECISION` · มติ DP-9) | ผลพิจารณามีชื่อ 3 ชุด (ปุ่ม/flow/ผลลัพธ์) → `code_name` / `code_mapping` / `other_value` · เปลี่ยนชื่อปุ่มตาม SDD GI ได้โดยไม่ deploy |
| `RunningNumber` | `sgi_document_running_numbers` | ออกเลข `YYYY/xxxxx` แบบ atomic ต่อปี — กันเลขชนเมื่อ batch + ผู้ใช้สร้างพร้อมกัน |
| `ImpactCostDetail` | `sgi_document_cost_details` | ยอดชดเชยแยกรายเดือน/รายร้านใหม่ — จำเป็นต่อการทวนยอดกับ Statement/SAP |
...
`CompensateHistory` → `sgi_consideration_logs` · `CompetInCompenProfile`/`CompetitionProfile` → `sgi_document_competitors`/`sgi_competitors` · `FactorInCompenProfile`/`FactorProfile` → `sgi_document_external_factors`/`sgi_external_factors` · `ImpactProfile` → `sgi_fgi_impact_stores` + `sgi_document_new_stores` · `StatusProfile` → `document_statuses` · `CommonConfig` → `mas_param` ของระบบ SBP เดิม (ไม่มีตาราง/หน้าจอ config ใน SGI) · `MaintainHistory`/`MaintainMasterHistory` → **ไม่ migrate** (ยกเลิกระบบ audit ของ master 2026-08-07) · `Journal`/`JournalDetail` → application log ของ BE (ไม่มีตาราง `job_run_histories` ใน target schema แล้ว) · `CompDocAttachment`/`CompTempAttachment` → `sgi_document_attachments` · `CompensateHistory.ActionWaitingDays` → `workflow_tasks.waiting_days`
...
| `LogCompensate` / `LogCompensateReturn` | log request/response ข้ามระบบ (FMS) — `sgi_interface_transactions` ครอบการรับส่งไฟล์/ข้อความกับระบบภายนอก และ **การส่งต่อภายใน (`direction = INTERNAL`)** ที่มาแทนไฟล์ EAI ของ Jobs 7/8/9 |
...
|---|---|
| `FGI_IMPACT_STORE` | `sgi_fgi_impact_stores` |
| `FGI_IMPACT_STORE_ON_PROCESS` | `sgi_fgi_impact_processes` (hub · ออก `impact_process_id`) |
| `FGI_IMPACT_STORE_SALES` | `sgi_fgi_impact_sales_summaries` |
| `FGI_IMPACT_STORE_SALES_TRN` | `sgi_sales_transactions` |
| `FGI_IMPACT_COMPETITOR` | `sgi_fgi_impact_competitors` |
| `FGI_CONFIRM_RECEIVE_DATA` | `sgi_interface_transactions` (typed FK แทน polymorphic — แก้ E20) |
| `FCS_QSSI_SCORE` | `fcs_qssi_score` |

> ### ✅ มติ 2026-08-21 — รับ F8 + F1 เข้าโครงแล้ว (19 → 20 ตาราง)
>
> **F1** → ตาราง **`sgi_fgi_impact_compensations`** (ยอดชดเชยรายงวด forecast/adjust)
> **F8** → เติม 5 กลุ่มคอลัมน์เข้า `sgi_fgi_impact_processes`: `last_compensate_seq` · `last_compensate_seq_no` · `start/end_compensate_month/year` · `flag_action` · `datasource`


#### 🔀 ช่องทางต้นทางของเคส — `sgi_fgi_impact_processes.datasource` (B2 · ตั้งรหัสใหม่ 2026-08-24)
...
>
> FE อ่านตัวเลือกจาก `common_code` ของระบบเดิม (`code_type = SGI_DATASOURCE`) ผ่าน `GET /common/common-code` — **ห้าม hardcode**
>
...
> }
> ```
> แปลว่า **งาน implement ฝั่งเราอยู่ที่ `LLDD-BE-Job-6-ExportImpactStoreToFS`** — Job 6 ต้องเขียน `sgi_fgi_impact_processes` (5 คอลัมน์ใหม่) และ `sgi_fgi_impact_compensations` เพิ่มจากงานเดิมที่ทำอยู่
>
...
|---|---|---|---|
| F1 | `FGI_IMPACT_STORE_COMPENSATE` (20 คอลัมน์) | **ยอดชดเชยรายงวดฝั่ง pipeline** ที่เกิด*ก่อน*มีเอกสาร — แยก `COMPENSATE_FORECAST` (ระบบคำนวณ) กับ `COMPENSATE_ADJUST` (คนปรับ) · `COMPENSATE_STATUS` · `COMPENSATE_COMMENT` · `STMT_MONTH`/`STMT_YEAR` (งวด statement) · `APPROVE_DATE` · `COMPENSATE_SEQ`/`SEQ_NO` (รอบ/ครั้งที่) — โครงเรามีแต่ยอดสุดท้ายบน `sgi_compensation_documents` | เพิ่ม `sgi_fgi_impact_compensations` (zone A) · `FLAG_SEND_BPM` ตัดทิ้งได้ (ไม่ export ไฟล์เข้า K2 แล้ว) แต่ **forecast vs adjust ต้องเก็บคู่กัน** เพราะ SDD GI ให้คีย์ปรับยอดได้ |
| F2 | `FGI_NEW_STORE_COMPENSATE` (19) | ของเดิมเก็บ **ยอด + % ทั้งฝั่ง forecast และ adjust** ต่อร้านเปิดใหม่ (`COMPENSATE_FORECAST_N` / `_PERCENT_N` / `COMPENSATE_ADJUST_N` / `_PERCENT_N`) — โครงเรามีชุดเดียว | ขยาย `sgi_document_new_stores` เป็น 2 ชุด (forecast/adjust) หรือแยกตาราง zone A · **ยืนยันการเพิ่ม `compensate_amount` ที่ทำไปแล้ววันนี้ว่าถูกทาง** |
...
| F6 | `FGI_WS_LOG` (7) | log **request/response ของ web service** (`DATA_INPUT`/`DATA_OUTPUT` CLOB · `ERROR_CODE`/`MSG` · start/end) — application log เก็บผลการรัน job และ `sgi_interface_transactions` เก็บการรับส่งไฟล์/ข้อความ แต่ไม่มีที่เก็บ payload ราย call | เพิ่ม `integration_call_logs` — ยังจำเป็นแม้ตัด K2 REST เพราะ QSSI/ALLMAP/IAS ยังเป็น service call · ซ้ำประเด็นกับ `LogCompensate` ในรายการ P1 ฝั่ง K2 |
| F7 | `FGI_CANCEL_PROCESS` · `FGI_CANCEL_APPROVE` · `FGI_CANCEL_HISTORY` · `FGI_CANCEL_MAIL` · `FGI_COMPETITOR_DEL` (5 ตาราง + sequence รายภาค `SEQ_FGI_CANCEL_PROCESS_00`–`09`) | **สายอนุมัติ "ขอยกเลิก" อีกชุดหนึ่งที่แยกจาก workflow หลัก** — มี `CANCEL_TYPE`/`CANCEL_SUBTYPE` · ช่วงวันที่ยกเลิก · `APPROVE_SEQ` หลายชั้น · ผู้เปิดเรื่อง (ชื่อ/หน่วยงาน/เบอร์/อีเมล) · mail TO/CC/BCC ต่อชั้น · `STATUS_ID` — **ไม่ถูกอ้างถึงเลยใน `fcsJar` และใน repo `SBP/` ทั้ง 3 ตัว** แปลว่าเป็นของ **เว็บ FCS เดิม** ที่อยู่นอกทั้งสองงาน | **ต้องถามธุรกิจ**: SGI รับงานนี้มาด้วยหรือไม่ · ไม่ใช่ตัวเดียวกับผลพิจารณา "หยุดชดเชยประกันรายได้" ในขั้น 06 ของ workflow เรา |
| F8 | `FGI_IMPACT_STORE_ON_PROCESS` (คอลัมน์ที่ยังไม่ครอบ) | `START_COMPENSATE_MONTH/YEAR` + `END_COMPENSATE_MONTH/YEAR` (กรอบงวดที่ชดเชยได้) · `LAST_COMPENSATE_SEQ_NO` · `FLAG_ACTION` · `DATASOURCE` | เติมคอลัมน์เข้า `sgi_fgi_impact_processes` — กรอบเริ่ม/จบงวดคือกติกา "เดือนที่ 4 หยุดชดเชย" และการเปิดเรื่องซ้ำตาม SDD GI |

### ขอบเขต — ของที่ **ไม่ใช่** ของ SGI (ห้ามทำซ้ำ)

- `MAS_STORE` · `MAS_STORE_TYPE` · `MAS_EMPLOYEE` · `MAS_STORE_COMPETITOR` · `FCS_MONTHLY_SALES` · `FML_SBP_STMT` → **master/ข้อมูลของระบบ FCS/SBP เดิม** (ดู `SBP/README.md` — store-backend เป็นเจ้าของฝั่ง PostgreSQL) · `stores`/`employees`/`branch_types`/`sgi_competitors` ในโครงเราคือ **read model ที่ sync มา ไม่ใช่ master**
  - `FML_SBP_STMT` มี `DOCUMENT_ID` + `CHANNEL_TRAN_ID` + `REPORT_LINK` — คือปลายทางของ `sgi_compensation_documents.statement_id` ที่เราตั้งไว้ ตรงกันแล้ว
- `V_FGI_SBP_APPROVER` → **auth-backend ของระบบ SBP เดิม** ตามการตัดสินใจ 2026-08-05 · สิ่งที่ต้องบันทึกไว้: view นี้ resolve ผู้อนุมัติจาก `business_user` + `business_user_group` (group_id = 21) โดย key คือ **(store_type, store_area)** แล้วจัดอันดับด้วย `position_level` — SGI ต้องส่ง 2 คีย์นี้ไปถามระบบเดิม และ `position_level` คือจุดที่แยก **ผู้รักษาการ** ออกจากผู้อนุมัติจริงตาม SDD GI
...
|---|---|---|
| `workflow_instances` · `workflow_tasks` | **`@srm/glb-workflow`** ใน schema **`sps_store`** — `workflow_transaction` (instance · 19,327 แถว) · `workflow_approver` (prepared approvers · 99,198 แถว) · `workflow_history` (timeline · 38,136 แถว) | SGI ขอ **workflow version ใหม่** 1 ตัว แล้วเรียก `initializeWorkflow` → `addPreApprover` → `eventWorkflow` · **✅ ชื่อ function ยืนยันแล้ว 2026-08-14** (API 8 ตัวตามชีต `Detail` ของ LLDD ฝั่ง lib) · **✅ DP-1 ปิดแล้ว 2026-08-17** — `referenceId` = `sgi_compensation_documents.id` (surrogate · ส่งเป็น string) |
| `workflow_sections` · `document_statuses` | **`workflow_state` / `workflow_route` / `workflow_status`** ของ engine ใน **`sps_store`** (definition เก็บใน DB ไม่ใช่โค้ด · ของจริงมี state 18 · route 43 · status 22 แถว) | 5 ขั้น 06/08/01/02/03 = state · การส่งต่อ/ตีกลับ = route · สถานะ 6 ค่า = status · **วงเงินอนุมัติ (เกณฑ์เดียว 100,000) เก็บใน `common_code`** (`code_type = SGI_APPROVE_LIMIT`) หรือ `workflow_route.condition_json` — ยังคงเป็น data ตาม SDD GI |
...
| `email_templates` | **`email_template`** (`email_template_id` · `subject_format` · `body_format`) + **`email_sent`** (log ทุกฉบับ) + lib `@gosoft-sbp/email-lib` (`emailId`) | 8 template EM-01–08 = **8 แถวใน `email_template` เดิม** · **ลบหน้าจอ `email-template.html` และ endpoint `/email-templates/*` ทั้งกลุ่ม 2026-08-06** — SGI แค่**อ่าน**ไปประกอบอีเมลแล้วส่งผ่าน lib การแก้ template ทำที่ระบบ SBP เดิม · ไม่ต้องทำ mail sender เอง |
| `system_configs` | **`sps_store.mas_param`** (`param_name` · `param_value` · `ref_name` · `description` · `is_config` · `active_flag`) — **ตาราง config กลางของ store-backend** 93,763 แถว · ⚠️ **ไม่มี PK/unique** (btree `(param_name, param_value)` เท่านั้น) จึงต้อง `WHERE active_flag='Y'` + `LIMIT 1` เสมอ · ⚠️ **มีเฉพาะ schema `sps_store` — `sps_auth` ไม่มีตารางนี้** | ถ้าต้องการ `category`/`value_type`/`is_editable` ให้**เพิ่มคอลัมน์ใน `mas_param`** ไม่สร้างตารางใหม่ · **ลบหน้าจอ `system-config.html` และ endpoint `/configs*` ทั้งกลุ่ม 2026-08-06** — SGI แค่**อ่าน**ค่าไปใช้ การแก้ค่าทำที่ระบบ SBP เดิม |

### มติจากการเทียบฐานข้อมูลจริง (ตัดสินใจ 2026-08-10 · DP-1 · DP-3 · DP-9)

| มติ | ผลต่อ schema |
|---|---|
| **DP-9 = แยกตัดสิน** | `decisions` **ตัดออก** → ใช้ `common_code` ของระบบเดิม (`code_type = 'SGI_DECISION'`) · `sgi_external_factors` + `sgi_competitors` **ยังเป็นตารางของ SGI** เพราะมีหน้าจอ CRUD ของตัวเอง และการเขียนลง lookup กลางไม่คุ้มความเสี่ยง (ตอนนั้น 20 ตาราง · ปัจจุบัน 19) |
| **DP-3 = ทางเลือกที่ 3 (ผสม)** | `sgi_impacted_stores` **ยังเป็นตาราง** แต่เป็น **snapshot บางส่วน** — เก็บเฉพาะร้านที่เคยเข้ารอบชดเชย เติมแบบ upsert ตอนสร้าง `sgi_fgi_impact_processes` ไม่ sync ทั้ง master |
| **DP-1 = ทางเลือก B** | `sgi_compensation_documents` เปลี่ยนเป็น **surrogate PK `id`** · `doc_no` เป็น **UNIQUE NOT NULL** (business key) · `reference_id` ที่ส่งให้ engine = `id` |
...
> ✅ **seed ลงฐาน dev จริงแล้ว 2026-09-16** — `common_code_type` 3 · `common_code` 14 · `mas_param` 11 ·
> `email_template` 8 · คู่แข่ง 11 · ปัจจัยภายนอก 7 · ตัวนับเลขเอกสาร 10 ปี
> (ยืนยันด้วย `tools/introspect_dev_sgi.py` · ติดตั้งด้วย `output/sql/sgi_seed_data.sql`)
>
> 🔴 **ค่าเหล่านี้เป็นแถวที่ SGI เพิ่มเอง ไม่ใช่ของเดิมที่ reuse ได้ทันที (ระบุชัด 2026-08-25):** `SGI_APPROVE_LIMIT` · `SGI_DECISION` · `SGI_DATASOURCE` และค่าใน `mas_param` (`SGI_IMPACT_RADIUS_*` · `SGI_SALES_DAYS_MIN` · `SGI_GROWTH_RATE_MAX`) เป็นแถวที่ **การออกแบบวางแผนจะเพิ่ม** ไม่ใช่ของเดิมที่ reuse ได้ทันที — งาน seed อยู่ใน `LLDD-BE-Data-Migration-Cutover` และมี SQL ตัวอย่างใน `LLDD-BE-Integration-SBP-Platform` 5.5.2
>
> 🔴 **กับดัก schema:** `mas_param` มี**เฉพาะ `sps_store`** (`sps_auth` ไม่มีตารางนี้เลย) · `common_code` มี**ทั้งสอง schema แต่คนละตาราง** — `sps_store` 14 คอลัมน์ 2,641 แถว vs `sps_auth` **13 คอลัมน์ 2,594 แถว** (ชุดเก่าของ auth-backend) · **SGI ใช้ `sps_store` เท่านั้น** และต้องเขียน schema นำหน้าใน SQL ทุกครั้ง (กับดักเดียวกับตาราง `workflow_*`)

> ⚠️ **ข้อควรระวังที่ต้องรับทราบ:** `common_code` **ไม่มี PK และไม่มี unique constraint** — มีแค่ index ธรรมดา `btree (code_type, code_value, code_name)` แปลว่า**ฐานข้อมูลไม่กันรหัสซ้ำให้** ต้องกันที่ระดับแอปพลิเคชัน หรือขอเพิ่ม partial unique index (`WHERE code_type='SGI_DECISION'`) ซึ่งต้อง sign-off จากทีมเจ้าของตาราง เพราะเป็นตารางที่ทุกโมดูลอ่าน (2,641 แถว)
> และต้องลงทะเบียน `code_type` ที่ `common_code_type` (378 แถว) ก่อนใช้งาน

### ตารางที่ "คล้ายแต่ไม่ใช่" — ตรวจแล้วต้องเก็บของเราไว้

| ตารางของเรา | ของ SBP ที่ใกล้เคียง | เหตุผลที่ยังต้องมี |
|---|---|---|
| `sgi_consideration_logs` | `workflow_history` (from_state → to_state · remark · user) | engine ไม่มี `decision_code` · `result_category` (APPROVE/REJECT — ใช้ filter รายงาน) · ไฟล์แนบต่อการพิจารณา → เก็บตารางเราไว้และผูก `transaction_id` ของ engine |
| `sgi_document_attachments` | `upload_general` (ไฟล์ของงาน general upload) + service `POST /statement/upload-file-aws` · `download-file-aws` | คนละบริบทไฟล์ · **แต่ให้ใช้ service อัปโหลด/ดาวน์โหลด S3 ของระบบเดิม ไม่เขียน storage layer เอง** · **เหตุผลจริงที่ต้องมีตารางเอง = `upload_general` ขาด `file_size` / `content_type` / `section_code` / `upload_status` / `purge_flag`** (ไม่ใช่เพราะติด FK `job_id` — ดูข้อแก้ความเข้าใจผิดในหัวข้อผลการเทียบ DB จริง) · **✅ DP-8 ปิดแล้ว 2026-08-24 — ทำตารางของเราเอง (เก็บ metadata) + ใช้ service S3 ของระบบเดิม** ไม่ต่อยอด `upload_general` |
| `sgi_interface_transactions` | `integration_log` (`module` · `service` · `payload`) | ของเราติดตาม ACK ระดับ record/ไฟล์ · `integration_log` เป็น payload log ราย call → **ใช้ `integration_log` แทนตาราง `FGI_WS_LOG` ในข้อ F6 ได้เลย ไม่ต้องเพิ่มตารางใหม่** |
...
ทั้งสอง schema มีครบ 13 ตารางเหมือนกัน (DDL ชุดเดียวกัน) แต่ **engine ตัวจริงที่ SGI ต้องเสียบคือ `sps_store`** — ดูจากปริมาณข้อมูล:

| ตาราง | `sps_store` (ตัวจริง) | `sps_auth` (ของ auth-backend คนละชุด) |
|---|---:|---:|
| `workflow_transaction` | **19,327** | 55 |
| `workflow_history` | **38,136** | — |
| `workflow_approver` | **99,198** | — |
...
ตารางนี้มี **19,327 แถว** แต่ใน schema dump **ไม่มี PRIMARY KEY และไม่มี index ใด ๆ** (ตัวเดียวกันใน `sps_auth` มี PK `transaction_id` ปกติ) · `workflow_state` · `workflow_event` · `workflow_part_display` ใน `sps_store` ก็ไม่มี PK/index เช่นกัน

| หัวข้อ | รายละเอียด |
|---|---|
| **ผลกระทบ** | ทุก query ที่ SGI ยิงหาเอกสารด้วย `reference_id` จะเป็น seq-scan บนตารางที่โตขึ้นเรื่อย ๆ · ไม่มีอะไรกันแถว duplicate ระดับฐานข้อมูล |
...
### 4. `fcs_qssi_score` — มีอยู่จริง 24,284,545 แถว ห้ามสร้างใหม่

ดูรายละเอียดในแถว `fcs_qssi_score` ของ Data Dictionary โซน A ด้านบน · สรุป: ชื่อ **เอกพจน์** · reuse ของเดิม · มี `POST /performance/import-qssi` + staging `fcs_tmp_qssi_score` ทำงานอยู่แล้ว · **✅ DP-4 ปิดแล้ว 2026-08-24 — ไม่แก้ตารางเดิม (NOT NULL / UNIQUE / index) เพราะ SGI อ่านอย่างเดียว**

### 5. `fcs_monthly_sales` ใช้แทน `sgi_sales_transactions` **ไม่ได้**

| | `fcs_monthly_sales` (ของเดิม) | `sgi_sales_transactions` (ของเรา) |
|---|---|---|
...
ยอดรายเดือน **ย้อนกลับเป็นรายวันไม่ได้** และการหา outlier ≥ 50 แบบจับคู่ต้องใช้รายวัน ⇒ ยังต้องสร้าง `sgi_sales_transactions` เอง · `fcs_monthly_sales` **ใช้เป็นข้อมูล cross-check ยอดรวมรายเดือนได้**

### 6. ปริมาณจริงของตารางที่ตกลง reuse

| ตารางของระบบเดิม (`sps_store`) | แถว | ใช้แทนอะไรในโครงเรา |
|---|---:|---|
| `mas_param` | 93,763 | `system_configs` |
| `common_code` | 2,641 | `branch_types` · **`decisions`** (มติ DP-9 2026-08-10 · `code_type = SGI_DECISION`) · วงเงินอนุมัติ `SGI_APPROVE_LIMIT` — **`sgi_external_factors`/`sgi_competitors` ไม่ย้ายมา** |
| `common_code_type` | 376 | นิยาม `code_type` ของข้างบน |
| `email_template` | 85 | `email_templates` |
| `email_sent` | 5,392 | log อีเมลทุกฉบับ |
| `business_user` | 12,759 | `employees` |
| `mas_store` | 19,647 | `stores` |
| `store` | 19,402 | `stores` |

ตารางเหล่านี้ทีมอื่นเขียนอยู่จริง ⇒ **การเพิ่มคอลัมน์/constraint ต้องขอ sign-off เจ้าของก่อน** ไม่ใช่งานที่ SGI ทำเองได้

### 7. แก้ความเข้าใจผิดในเอกสารเก่าของเรา (2 จุด)

| จุด | ที่เคยเขียนไว้ (**ผิด**) | ข้อเท็จจริงจาก schema จริง |
|---|---|---|
| `sgi_document_attachments` | "ถ้าใช้ `upload_general` จะติด FK `job_id`" | **ไม่จริง** — `upload_general.job_id` และ `audit_log_id` เป็น **nullable ทั้งคู่** (FK ไป `general_upload_data_page_job` / `..._audit_log`) · **เหตุผลจริงที่ต้องมีตารางเอง คือขาด `file_size` · `content_type` · `section_code` · `upload_status` · `purge_flag`** |
...
| # | เรื่อง | สถานะ |
|---|---|---|
| DP-1 … DP-12 | **12 ข้อ · ปิดแล้ว 8 (DP-1 · DP-3 · DP-9 เมื่อ 2026-08-10 · DP-5 · DP-10 · DP-4 · DP-7 · DP-8 ปิด 2026-08-24) เหลือค้าง 4** — DP-2 (ไม่มี PK/index บน `sps_store.workflow_transaction`) · DP-6 (`sgi_interface_transactions` ออกแบบใหม่หรือลอก `statement_summary`) · DP-11 (ตัวเลขเงินประกันรายได้เก็บที่ไหน) · DP-12 (audit ของ master เอากลับมาหรือไม่) | **ยังไม่ตัดสิน** — รายละเอียดครบทุกข้อพร้อมข้อดี/ข้อเสียอยู่ที่ [`SBP/SBPGI-vs-existing-system.md`](SBP/SBPGI-vs-existing-system.md) §4 · **เอกสารนี้บันทึกไว้เฉย ๆ ไม่เปลี่ยนดีไซน์ตามข้อเสนอ** |
...
|---|---|---|
| `BPM06001O_` | 48 | หัวเอกสาร + งวด + growth + ยอดชดเชย · **24 ฟิลด์ (ครึ่งไฟล์) เป็นบล็อกผู้อนุมัติ DV/GM/AVP** (emp id · ชื่อ-สกุล ไทย/อังกฤษ · อีเมล) → ยืนยันว่า `sgi_compensation_documents.approver_snapshot` ต้องมี **DV** ด้วย (ข้อ F3) · ฟิลด์ 9 = `allmap_url` และฟิลด์ 10 = ลิงก์ SBP Statement (`statement_id`) มาจากไฟล์นี้ตรง ๆ |
...
>
> 📌 **DP-13 นิยามอยู่ที่นี่ (ไฟล์นี้) ไม่ใช่ในทะเบียน DP-1…DP-12 ของ [`SBP/SBPGI-vs-existing-system.md`](SBP/SBPGI-vs-existing-system.md)** — ทะเบียนนั้นเป็นเอกสารของระบบเดิมที่เราไม่แก้ · DP-13 เกิดใหม่จากมติเปลี่ยนคำเรียกประเภทร้าน 2026-08-20 จึงต่อเลขไว้ในชุดเดียวกันเพื่อไม่ให้สับสน แต่บันทึกไว้ฝั่ง SGI
>
...
### PDPA — ข้อมูลส่วนบุคคลที่ตารางของ SGI เก็บจริง (SDD สไลด์ 63)
...
| ตารางของ SGI | คอลัมน์ที่เป็นข้อมูลบุคคล | ลักษณะ |
|---|---|---|
| `sgi_consideration_logs` | `consider_by` | รหัส/ชื่อผู้พิจารณา — อ้าง `business_user` ของระบบเดิม |
| `sgi_compensation_documents` · `sgi_fgi_impact_compensations` | `created_by` · `updated_by` | ผู้สร้าง/ผู้แก้ไข |
| `sgi_compensation_documents` | `approver_snapshot` (JSONB) ⏳ **ยังไม่มีใน DDL** | **ชื่อ + อีเมล** ของ FC/Section/Manager/GM/AVP ณ เวลาเปิดเอกสาร |
| `sgi_document_attachments` | `file_name` | ชื่อไฟล์อาจมีชื่อบุคคล |
...
|---|---|---|
| วงเงินอนุมัติ | `SectionProfile.SectionLimitCost` มีค่าเดียวคือ section 2 (GM) = **100,000** · AVP เป็น NULL | เป็นเกณฑ์**เก่า** — SDD GI เปลี่ยนเป็น เกณฑ์เดียว 100,000 · ห้าม migrate ค่าเดิมมาตรง ๆ ให้ seed ใหม่ที่ `common_code` (`SGI_APPROVE_LIMIT`) |
...
| `sgi_interface_transactions` | PK `id`, เวลา ACK `acked_at` | API อาจ alias เป็น `trackingId`/`receiveDate` แต่ SQL ต้องอ้าง `id`/`acked_at` |
| `sgi_fgi_impact_processes` | `workflow_generation_status` | ห้าม duplicate สถานะเดียวกันใน `sgi_fgi_impact_stores` |
...
1. **`sgi_impacted_stores.store_code = sgi_fgi_impact_stores.impacted_store_code`** — สะพานหลักโซน C (K2) ↔ โซน A (FGI/FCS) · รหัสร้าน 5 หลักเดียวกันทั้งระบบ
   - **`stores`** = master สาขา 7-Eleven ทุกประเภท · `sgi_impacted_stores` เป็น subset ร้าน SP · ร้านเปิดใหม่ (`sgi_document_new_stores.new_store_code`) อ้าง `stores` ตัวเดียวกัน — เป็นแหล่งของ popup ค้นหาร้านในหน้าสร้างเอกสาร
2. **`*.impact_process_id → sgi_fgi_impact_processes.id`** — hub กลางของคู่ร้าน ยอดขาย และคู่แข่งในหนึ่งรอบชดเชย (ใหม่)
3. **`sgi_compensation_documents.impact_process_id → sgi_fgi_impact_processes`** — FK ใหม่ **1 รอบชดเชย : 1 เอกสาร** แทนการส่งไฟล์ BPM06001O (48 ฟิลด์) ข้ามระบบ (ใหม่)
...
2. **บังคับ status domain ด้วย enum / check constraint** — `verify_status` W/P/N · `sales_request_status` W/P/Y/E · `sales_status` W/P/Y/N/E · `compensate_status` I/C/A/N/S/Z (**`C` = ร้านปิด/สัญญายกเลิก · ส่ง STA เป็น `S` เหมือน `Z`** · ปิด DP-14 2026-09-02) · action_status (Y/W/N) · workflow_generation_status (W/Y/N)
...
- **SQL ติดตั้งจริง:** [`output/sql/sgi_schema.sql`](output/sql/sgi_schema.sql) (สร้าง **20 ตารางใหม่ + 27 index** ลง schema `sps_store` ของระบบ SBP เดิม · รวม `fcs_qssi_score` ที่ reuse = **21 ตารางในขอบเขต**) และ [`output/sql/sgi_seed_data.sql`](output/sql/sgi_seed_data.sql) (ข้อมูลตั้งต้น incl. `common_code` / `mas_param` / `email_template` ของระบบเดิม — **INSERT อย่างเดียว**) · สร้างด้วย `python3 tools/build_sgi_schema_sql.py` จาก DDL ชุดเดียวกับเอกสาร **ห้ามแก้ .sql ด้วยมือ**
