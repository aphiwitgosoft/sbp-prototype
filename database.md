...
| `LogCompensate` / `LogCompensateReturn` | log request/response ข้ามระบบ (FMS) — `sgi_interface_transactions` ครอบไฟล์/ACK กับระบบภายนอก และ **การส่งต่อภายใน (`direction = INTERNAL`)** ที่มาแทนไฟล์ EAI ของ Jobs 7/8/9 |
...
| F6 | `FGI_WS_LOG` (7) | log **request/response ของ web service** (`DATA_INPUT`/`DATA_OUTPUT` CLOB · `ERROR_CODE`/`MSG` · start/end) — application log เก็บผลการรัน job และ `sgi_interface_transactions` เก็บไฟล์/ACK แต่ไม่มีที่เก็บ payload ราย call | เพิ่ม `integration_call_logs` — ยังจำเป็นแม้ตัด K2 REST เพราะ QSSI/ALLMAP/IAS ยังเป็น service call · ซ้ำประเด็นกับ `LogCompensate` ในรายการ P1 ฝั่ง K2 |
