...
| 1.5 | **ยอดชดเชย 0**: เดือน 1–3 ส่งต่อ ~~01~~ **08** · เดือนที่ 4 หยุดชดเชย | ✅ | สไลด์ 51 · `k2-document.html` แผงพิจารณาขั้น 06 แสดงกติกาเสมอ · ยอด = 0 → ไฮไลต์ตัวเลือกที่ต้องกดตามครั้งที่ <br>⚠️ **ปรับปลายทางแล้วเมื่อ 2026-09-01: เดือน 1–3 เข้า `08` ไม่ใช่ `01`** — บรรทัดนี้เป็นบันทึกรีวิว ณ วันนั้น เก็บไว้เป็นประวัติ · ยึด `workflow.md` เป็นแหล่งความจริง |
...
| D | **Performance Index มาจาก QSSI** — เป็นตัวตั้งของการคำนวณ | 33 · 36 · 40 | ⚠️ | มี **`fcs_qssi_score`** (เอกพจน์ · คะแนน 6 หมวดดิบ — **ตารางนี้มีอยู่จริงแล้วใน `sps_store` 24,284,545 แถว พร้อม import pipeline ที่ทำงานอยู่ ห้ามสร้างใหม่ ให้ reuse**) แต่ยังไม่มีตัว index ที่ derive ออกมา |
...
| 6.4 | **AVP (03) เห็นควรไม่ชดเชย** | ปิดแล้ว (มติ 2026-09-02) — ขั้น 03 จบ flow ทันทีเหมือน 01/02 · SDD GI ไม่ได้ไล่ถึงขั้น AVP ไม่ใช่ตั้งใจให้ต่างกัน · การให้ 06 แก้ข้อมูลใช้ปุ่ม "ส่งกลับฝ่าย SBP DSA" แทน |
...
- **เทียบ DB เดิมของ K2** (`script_TB_DB_CPA_FRN_FGI_20260722.sql`) → เพิ่ม 5 ตาราง (`zones`, `branch_types`, `decisions`, `sgi_document_running_numbers`, `sgi_document_cost_details`) + คอลัมน์ `approve_limit_amount`, `round_no`/`loop_no`, `allmap_url`, `statement_id`, `approver_snapshot` → รวม 34 ตาราง / 47 endpoint · **แล้วตัดอีก 10 ตาราง / 3 endpoint เมื่อ 2026-08-06 (รอบ 2) เพราะระบบ SBP ปัจจุบันมีอยู่แล้ว → เหลือ 24 ตาราง / 47 endpoint** · **แล้วลบอีก 2 ตาราง / 16 endpoint เมื่อ 2026-08-06 (ลบหน้า Global Config + Email Template ทั้งฟีเจอร์ 10 เส้น และตัด 2 tab ควบคุมของหน้า Batch Job ทำให้ตัด 6 เส้น + `job_configs`/`job_run_histories`) → เหลือ 22 ตาราง / 31 endpoint** · แล้วตัด `audit_logs` + `GET /audit-logs` เมื่อ 2026-08-07 → เหลือ **21 ตาราง / 30 endpoint** · แล้ว **มติ DP-9 (2026-08-10)** ย้าย `decisions` ไป `common_code` → ตัด 1 ตาราง + 1 endpoint (`GET /decisions` · Lookup 3 → 2) → เหลือ **20 ตาราง / 29 endpoint** · แล้ว **มติข้อ 2.13 (2026-09-08)** ตัด `POST /sgi/interface/sta/ack` (สเปก STA ไม่มี ACK แบบ HTTP · Interface 3 → 2) · แล้ว **มติผู้ใช้ (2026-09-13)** เพิ่ม `sgi_fgi_new_store_compensations` (ค่าชดเชยฝั่งร้านเปิดใหม่ **รายงวด** · ORA `FGI_NEW_STORE_COMPENSATE` 23,628 แถว — `sgi_fgi_impact_stores` ไม่มีมิติงวดจึงเก็บแทนไม่ได้) → **ตัวเลขปัจจุบัน 21 ตาราง / 28 endpoint 6 กลุ่ม** (Lookup 2 · Master Data 8 · เอกสาร 11 · รายงาน 2 · Workflow 3 · Interface 2)
...
- ~~ย้าย **ตั้งค่าระบบ / Batch Job / Email Template** ไปกลุ่มเมนู **ผู้ดูแลระบบ (Admin)**~~ → **ยกเลิกทั้งข้อเมื่อ 2026-08-06 (รอบ 3)**: กลุ่ม **Admin ถูกลบทั้งกลุ่ม** · `system-config.html` และ `email-template.html` **ถูกลบทิ้งพร้อม 10 endpoint** (ระบบ SBP เดิมดูแล `mas_param` / `email_template` อยู่แล้ว SGI แค่อ่าน) · `job-batch.html` **ย้ายไปกลุ่ม `Flow` เป็นหน้าอ้างอิงชื่อ "Flow Batch Job"** เหลือ 2 แท็บ (Flowchart + Database ที่ใช้) ตัดแท็บควบคุม/ประวัติการรัน + 6 endpoint + 2 ตารางออก · เมนู **สร้างเอกสาร** อยู่บนสุดของกลุ่มระบบประกันรายได้
...
| K | **engine ตัวจริงอยู่ schema `sps_store` ไม่ใช่ `sps_auth`** — สอง schema มีครบ 13 ตารางชื่อเดียวกันแต่คนละชุดคนละเวอร์ชัน (`workflow_state` 3 vs 4 คอลัมน์) · `sps_store`: transaction 19,327 · history 38,010 · approver 96,542 · `sps_auth`: transaction 55 · route 41 · state 10 | ต้องระบุ schema ให้ชัดทุกจุดที่อ้าง engine |
| L | ⚠️ `sps_store.workflow_transaction` **ไม่มี PK และไม่มี index เลย** ทั้งที่มี 19,327 แถว (ตัวเดียวกันใน `sps_auth` มี PK) | **ความเสี่ยงที่ต้องคุยกับทีมเจ้าของ library** · ยังไม่ตัดสิน → **DP-2** |
...
| S | ปริมาณจริงของตาราง reuse (วัดจากฐาน dev 2026-09-16): `mas_param` 93,763 · `common_code` 2,641 · `common_code_type` 380 · `email_template` 134 · `email_sent` 5,392 · `business_user` 12,759 · `mas_store` 19,647 · `store` 19,402 | ใช้ประกอบการประเมินผลกระทบตอน reuse |
