...
**4 เส้นที่มีแท็บ Flowchart:** `POST /documents/{docNo}/actions` (routing 7 ขั้น + กฎ 100,000) · `POST /workflows/instances` (Gen Flow Gate) · `POST /documents` (สร้าง + กันซ้ำ) · `POST /jobs/{jobNo}/run` (guard กันรันซ้อน)
...
| POST | `/documents/{docNo}/actions` | ส่งผลพิจารณา — หัวใจ workflow 7 ขั้น · กฎ 100,000 (มี Flowchart) |
...
| GET | `/workflow-sections` | รายการ Section 7 ขั้น — dropdown ตำแหน่ง/ตัวกรอง |
...
### 7. รายงาน · K2 3.1.7 (2 เส้น)
| Method | Path | ทำอะไร |
|---|---|---|
| GET | `/reports/status-summary` | รายงานสรุปสถานะ 19 คอลัมน์ — **บังคับระบุปี** · filter `result` (**ใหม่ session นี้** — อนุมัติ/ไม่อนุมัติ) |
| GET | `/reports/status-summary/export` | export .xlsx เงื่อนไขเดียวกัน (รวม `result`) |
...
- **กฎวงเงิน 100,000** ใน `/documents/{docNo}/actions`: ชดเชย/ไม่ชดเชย > 100,000 → AVP (03) · ชดเชย ≤ 100,000 → ฝ่ายบัญชี SBP (04) · 06 ไม่ชดเชย/หยุด → เสร็จสิ้น (ดูตารางเต็มใน `workflow.md`)
- **filter `result`** ใน report = อนุมัติ/ไม่อนุมัติ/ยังไม่พิจารณา อิง **ผลพิจารณาล่าสุด** (`consideration_logs.result_category` = APPROVE/REJECT/PENDING) — **ไม่ใช่เฉพาะขั้น 05** (เคยลองเฉพาะ 05 แล้ว revert กลับ)
