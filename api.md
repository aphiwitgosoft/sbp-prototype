...
- Code namespace ต้องไม่ตีความปนกัน: `roleCode` = RBAC role 00/01/02/03/04/05/06/10, `sectionCode` = workflow section 06/08/01/02/03, `statusCode` = document status lookup; ทุก response ที่ต้องแสดงสถานะให้ส่ง code เป็น canonical value แล้ว FE resolve label จาก `/sgi/lookup/document-statuses`
- **Batch Job — ตัด API ทั้งกลุ่ม (2026-08-06):** กลุ่ม Batch Job Admin 6 เส้น (`/jobs*`) ถูกลบออกจากแบบ · หน้า `job-batch.html` **ย้ายไปกลุ่มเมนู `Flow` ชื่อ "Flow Batch Job" และเหลือเฉพาะ 2 แท็บ `Flowchart การทำงาน` + `Database ที่ใช้`** (ตัดแบบฟอร์มพารามิเตอร์ · ประวัติการรัน · ปุ่มสั่งรัน/เปิด-ปิด job · stat cards · กราฟ · การ์ด audit ออกทั้งหมด) — เป็นเอกสารอ้างอิงสำหรับผู้พัฒนา ไม่ใช่หน้าจอควบคุม · **batch job ของ SGI ยังทำงานตามปกติ** แต่พารามิเตอร์/ตารางเวลากำหนดใน **backend config** (config file/env ฝั่ง BE) และผลการรันเก็บที่ application log / `sgi_interface_transactions` แทน ไม่มีหน้าจอและไม่มี API แก้ค่า
...
| Auth | ผู้ใช้ login ผ่าน BFF ระบบเดิม (Cognito · token เก็บใน httpOnly cookie ฝั่ง BFF) — SGI ตรวจ `x-api-key` + อ่าน user context จาก header ที่ BFF แนบมา; internal workflow/batch callback ใช้ service token | ระบบเดิม (BFF + auth-backend) จัดหา identity/สิทธิ์; BE validate header; FE ไม่แตะ token |
...
| Workflow transition | `/sgi/document/{docNo}/actions` รับ `{result,comment}` โดย `result` เป็น **7-enum** ไทย verbatim (ค่า "ส่งฝ่ายส่งเสริมธุรกิจ SBP" เปลี่ยนชื่อเป็น "ส่งหน่วยงานส่งเสริมธุรกิจ SBP" ตาม SDD GI 24/02/2026 · เพิ่มค่าที่ 7 **"คำนวณเงินชดเชยเรียบร้อย"** เมื่อ 2026-09-01) และคืน `{nextSection,statusCode,message}`; positive path คือ **`06→08→06→01→02→03→99`** (มติ 2026-09-01 — Section 08 คำนวณเสร็จคืนงานให้ 06) โดย `99` = เสร็จสิ้นและ `nextSection=null`; ที่ Section 02 ยอด < 100,000 จบเป็น 99 โดยไม่ผ่าน 03 · ≥ 100,000 → 03 (SDD GI); **`result = "ส่งกลับ"` คืน `nextSection = "06"` เสมอทุก Section** (มติ 2026-09-01) | BE Workflow Action เป็น source of truth; FE ไม่คำนวณ route เอง |
| RBAC/Menu | sidebar/route guard ใช้ของระบบเดิม: BFF `GET /menus` + `GET /groups/current-user/permissions` (`canView/canManage/canExport/canOther` ต่อ URL) · `GET /sgi/document/{docNo}` ยังคืน `permissions.canEditSections` และ `permissions.canAction` (ธงเชิง workflow ที่ SGI คำนวณเอง) | ระบบเดิมคุมเมนู/สิทธิ์เข้าหน้า · BE task-owner guard · FE เปิด/ปิด UI |
| Audit | **ไม่มี audit กลางของ master แล้ว** (ยกเลิก `audit_logs` 2026-08-07 · mutation master ไม่ต้องส่ง `reason`); workflow action ลง `sgi_consideration_logs`; batch เขียน application log | BE ไม่ต้องมี audit service · FE ไม่มีช่องเหตุผล |
...
**3 เส้นที่มีแท็บ Flowchart:** `POST /sgi/document/{docNo}/actions` (routing 5 ขั้น + วงเงิน เกณฑ์เดียว 100,000) · `POST /sgi/workflow/instances` (Gen Flow Gate) · `POST /sgi/document` (สร้าง + กันซ้ำเฉพาะเอกสาร active)

## Base path + กลุ่ม path ของ SGI (มติ 2026-08-25)

**ทุก endpoint ของงานประกันรายได้อยู่ใต้ `sgi` แล้วแตกเป็น 6 กลุ่มย่อยตามกลุ่มงาน** — ตรงกับ 6 กลุ่มในหัวข้อถัดไปแบบ 1:1

| กลุ่ม | prefix ของ API | route ของหน้าจอ | เส้น |
|---|---|---|---|
| 1. งาน & เอกสารประกันรายได้ | `/api/v1/sgi/document/*` | `/sgi/document/*` | 11 |
| 2. ข้อมูลอ้างอิง (Lookup) | `/api/v1/sgi/lookup/*` | — (ไม่มีหน้าจอดูแล) | 2 |
...
**ทำไมต้องมี prefix:** มติ **DP-10** ให้ SGI อยู่ใน **`srm-sps-spsap-store-backend` ตัวเดิม** ไม่แยก backend ใหม่ — และ backend ตัวนั้น**มีเส้นชื่อใกล้กันอยู่ก่อนแล้ว**:
...
- ฝั่ง NestJS ประกาศ **`SgiModule` เดียว** ผูก prefix ที่ระดับโมดูล (`RouterModule.register([{ path: 'sgi', module: SgiModule }])`) แล้วแตกเป็น **6 controller ตามกลุ่ม** (`DocumentController` · `LookupController` · `MasterController` · `ReportController` · `WorkflowController` · `InterfaceController`) — **ห้ามเติม `sgi/` ในแต่ละ `@Controller()`** จะได้ย้าย/เปลี่ยน prefix ที่เดียว
...
- เส้นที่ **ไม่ใช่ของ SGI ไม่ต้องใส่ prefix** และห้ามแตะ: `GET /store/search` · `GET /store/all-regions` · `GET /common/common-code` · `GET /menus` · `GET /groups/current-user/permissions` · `POST /statement/upload-file-aws` · `GET /api/workflow/pending` — เป็นของระบบ SBP เดิม เรียกตรงตามเดิม
- BFF ส่งต่อทั้ง prefix (`/api/v1/sgi/*`) โดยไม่ตัดคำ · สิทธิ์เมนูยังผูกกับ URL ของ **หน้าจอ** (`/sgi/<กลุ่ม>/...`) ไม่ใช่ URL ของ API
- `tools/check_docs.py` มีกฎกันหลุด 2 ข้อ: **ต้องอยู่ใต้ `/api/v1/sgi/`** และ **ต้องอยู่ในกลุ่มใดกลุ่มหนึ่งใน 6 กลุ่ม**
...
ไม่มี endpoint ใน SGI — FE ใช้ของระบบเดิมผ่าน BFF: login redirect `{bffUrl}/auth/login` (Cognito · cookie httpOnly) · `POST /auth/refresh` (axios interceptor เดิม) · `GET /auth/profile`, `GET /users/current` (ข้อมูลผู้ใช้) · `GET /menus` + `GET /groups/current-user/permissions` (sidebar/สิทธิ์ต่อ URL) — เส้นเดิมของกลุ่มนี้ (`/auth/login` `/auth/refresh` `/auth/me` `/me/menus`) ดูหัวข้อ "เส้นที่ตัดออก" ท้ายไฟล์

### 2. งาน & เอกสารประกันรายได้ · K2 3.1.2/3/4/6 (11 เส้น)
| Method | Path | ทำอะไร |
|---|---|---|
| GET | `/sgi/document/tasks` | งานรอท่านดำเนินการ (inbox ของ section — k2-list-waiting) |
| GET | `/sgi/document` | ค้นหาเอกสารที่เกี่ยวข้อง — **บังคับระบุปี** · filter: สถานะ · ภาค · ประเภทร้าน · ช่วงวันที่สร้าง · ช่วงยอดขายที่ลดลง/เงินชดเชย/วันที่รอ · **`result` (ประกันรายได้ / ไม่ประกันรายได้ / ยังไม่มีผล — เพิ่ม 2026-08-06)** อิง `sgi_consideration_logs.result_category` ล่าสุด (APPROVE/REJECT · เอกสารที่ยังไม่จบ = ไม่มีค่า) |
| GET | `/sgi/document/{docNo}` | เอกสารฉบับเต็ม 12 ส่วน + ธงสิทธิ์แก้ต่อ role/section |
| POST | `/sgi/document` | สร้างเอกสาร — ออกเลข YYYY/xxxxx + เปิด workflow (มี Flowchart) · **ตัดสินใจ 2026-08-06: ไม่มีฟอร์มสร้างเอกสารใน FE แล้ว** — ต้นทางสร้างที่ระบบ **FS** แล้วรอ **SBP Statement** ส่งข้อมูลกลับ (~1 วัน) จึงเรียกเส้นนี้โดย pipeline/service token · หน้า `k2-create.html` เหลือเป็นหน้าอธิบายกระบวนการ · การคีย์/ปรับข้อมูลร้านตาม SDD GI ทำในหน้าเอกสาร (`PUT /sgi/document/{docNo}`) |
| PUT | `/sgi/document/{docNo}` | บันทึกส่วนย่อย (ร้านใหม่/คู่แข่ง/ปัจจัย) · **%ชดเชยรวม = 100%** |
| POST | `/sgi/document/{docNo}/actions` | ส่งผลพิจารณา — หัวใจ workflow 5 ขั้น · วงเงิน เกณฑ์เดียว 100,000 · SDD GI (มี Flowchart) |
| GET | `/sgi/document/{docNo}/timeline` | ประวัติพิจารณาทุกขั้น |
| POST | `/sgi/document/{docNo}/attachments` | แนบไฟล์ ≤ 5MB |
| GET | `/sgi/document/{docNo}/attachments/{attachId}/download` | ดาวน์โหลดไฟล์แนบรายไฟล์ผ่าน BE authorization + AV clean guard · ไฟล์จริงใช้ service S3 ของระบบ SBP เดิม |
| GET | `/sgi/document/{docNo}/attachments/download-all` | **(เพิ่ม 2026-08-06)** ดาวน์โหลดไฟล์แนบทั้งหมดเป็น `.zip` — ปุ่ม “ดาวน์โหลดทั้งหมด” ระดับการ์ด (เทียบเท่าปุ่ม `Download` ของ K2 เดิม) · 404 เมื่อไม่มีไฟล์แนบ |
| GET | `/sgi/document/{docNo}/sales` | ยอดขาย 4 หน้าต่าง × 15 วัน — ใช้กับปุ่ม "ข้อมูลยอดขายเพิ่มเติม" · **กราฟแนวโน้มยอดขายรายวันในหน้าเอกสารถูกถอดออก 2026-08-06** เหลือเป็นข้อมูลประกอบการตรวจสอบและลิงก์ออก QlikView BI |
...
| Method | Path | ทำอะไร |
|---|---|---|
| GET | `/sgi/lookup/document-statuses` | รายการสถานะเอกสาร — dropdown ตัวกรอง (ค้นหา/รายงาน) |
| GET | `/sgi/lookup/workflow-sections` | รายการ Section 5 ขั้น (06/08/01/02/03) + `approveLimitAmount` ต่อขั้น — dropdown ตำแหน่ง/ตัวกรอง · FE ใช้แสดงวงเงิน ไม่ hardcode |

> **✅ มติ DP-9 (2026-08-10) — แยกตัดสิน:** `decisions` **ย้ายไปใช้ `common_code`** ของระบบเดิม (`code_type = SGI_DECISION`) จึง**ตัดเส้น `GET /decisions` ออก** — FE เรียก `GET /common/common-code?codeType=SGI_DECISION` ของระบบเดิมแทน · ส่วน `sgi_external_factors` และ `sgi_competitors` **ยังเป็นตารางของ SGI ตามเดิม** เพราะมีหน้าจอ CRUD ของตัวเอง และช่อง remark ของ `common_code` จำกัด 50 ตัวอักษร ไม่คุ้มที่จะไปแก้ตารางที่ทุกโมดูลใช้ร่วม · ⚠ `common_code` ไม่มี unique constraint → กันรหัสซ้ำที่ระดับแอป และลงทะเบียน `code_type` ที่ `common_code_type` ก่อน
...
| Method | Path | ทำอะไร |
|---|---|---|
| GET `/sgi/master/factors` · POST `/sgi/master/factors` · PUT `/sgi/master/factors/{code}` · DELETE `/sgi/master/factors/{code}` | **4 เส้น** | ปัจจัยภายนอก (sgi_external_factors) — รหัสห้ามซ้ำ · *ไม่มี `GET /sgi/master/factors/{code}` และไม่มี `PUT`/`DELETE` ที่ระดับ collection* |
| GET `/sgi/master/competitors` · POST `/sgi/master/competitors` · PUT `/sgi/master/competitors/{code}` · DELETE `/sgi/master/competitors/{code}` | **4 เส้น** | **master แบรนด์ร้านคู่แข่ง 11 รายการ** (รหัส 01–11 · ชื่อไทย+อังกฤษ) — `code` + `nameTh` + `nameEn` บังคับ · รหัสห้ามซ้ำ · หน้าจอ `k2-competitors.html` (**ใหม่ 2026-08-06** ตามหน้าจอ K2 เดิม) · `GET` เป็นแหล่งของ dropdown "ร้านคู่แข่ง (Master)" ในหน้าเอกสารด้วย · ต่างจาก `sgi_document_competitors` ที่เก็บ**รายสาขา**พร้อมรหัสจาก ALLMAP (ดู `docs/K2-interface-files.md`) |

> **กติกาการจัดกลุ่ม (ปรับให้ตรงกันทั้ง `api.md` และ `plan-api.html` · 2026-08-06):** master ที่**มีหน้าจอดูแลของตัวเอง** (ปัจจัยภายนอก → `k2-factors.html` · รายชื่อคู่แข่ง → `k2-competitors.html`) ให้เก็บ **CRUD ทั้งชุดไว้ในกลุ่ม Master Data** แม้ `GET` จะถูกใช้เป็น dropdown ในหน้าเอกสารด้วยก็ตาม — ส่วนกลุ่ม **Lookup** เหลือเฉพาะรายการอ้างอิงที่**อ่านอย่างเดียวและไม่มีหน้าจอดูแล** (`/sgi/lookup/document-statuses` · `/sgi/lookup/workflow-sections` · `/decisions`) เดิมทั้งสองไฟล์จัด `/sgi/master/competitors` คนละกลุ่มกัน ทำให้ตัวเลขต่อกลุ่มไม่ตรง

> **✅ มติ DP-9 (2026-08-10):** ทั้ง 8 เส้นของกลุ่มนี้ผูกกับ `sgi_external_factors` และ `sgi_competitors` ซึ่ง**ยังเป็นตารางของ SGI ตามเดิม** ไม่ย้ายไป `common_code` — เฉพาะ `decisions` เท่านั้นที่ย้าย
...
| GET | `/sgi/report/status-summary/export` | **Export Excel** — ส่งออกผลการค้นหา 14 คอลัมน์เป็น Excel ให้ทีมบัญชีนำไปกระทบ SAP · เงื่อนไข filter เดียวกับ `/sgi/report/status-summary` |
...
- **ข้อยกเว้นเดียวของกติกา ค.ศ.:** ไฟล์ที่ส่งให้ STA (`FRBC0001_…`) ใช้ **พ.ศ. + windows-874** (🔴 แก้ 2026-09-16 จากไฟล์ตัวอย่างจริง — ไฟล์ของ IAS `AMS06001O_`/`AMS06001I_` เป็น **ค.ศ.** และเป็น ASCII ล้วน เอกสารเดิมที่เขียนว่า IAS เป็น พ.ศ. นั้นผิด · ดู `docs/IAS-STA-interface-files.md`) และ **message ที่ส่งไป STA** (RabbitMQ exchange `sgi.interface` · JSON UTF-8 · ชุด `sgi_impact_store` / `sgi_reflow` ตามสัญญา `FRBC0001` เดิม) ยังคง **ฟิลด์วันที่เป็น พ.ศ.** ตามสัญญาเดิมของระบบปลายทาง — แปลงเฉพาะตอนอ่านไฟล์/ประกอบ payload เท่านั้น ห้ามให้ปนเข้ามาใน DB/API · *ตัว windows-874 หายไปพร้อมกับไฟล์ `FRBC0001` เมื่อย้าย STA ไป RabbitMQ (มติ 2026-08-24) — เหลือรอยืนยันว่าจะเปลี่ยนฟิลด์วันที่เป็น ISO ค.ศ. ได้หรือไม่*
...
- เส้นที่แก้**เอกสาร** ยังบันทึกผู้ทำและผลพิจารณาลง `sgi_consideration_logs` ตามเดิม (นี่ไม่ใช่ audit ของ master)
...
| 3 | **สาขาที่ถูกกระทบ และสาขาที่เปิดใกล้เคียง** | `sgi_document_new_stores` · `sgi_fgi_impact_stores` |
| 4 | **กรณีร้าน Take** — ตรวจเพิ่มสำหรับ "สาขาที่ถูกกระทบ" ที่เป็นร้าน Take | `sgi_impacted_stores` |
| 5 | **Center ตรวจจากรายงานประกันรายได้ของระบบ** แล้วกระทบยอดกับ SAP · Post คู่บัญชีถูกต้อง | `GET /sgi/report/status-summary` |
...
**เหตุผล:** ทั้งสองกลุ่มออกแบบไว้เพื่อรองรับหน้าจอ admin 2 หน้านี้โดยตรง และ**เขียนลงตารางของระบบ SBP เดิมอยู่แล้ว** (`mas_param` / `email_template`) ซึ่งระบบเดิมมีหน้าจอบริหารจัดการของตัวเองอยู่ — SGI จึงไม่ต้องทำหน้าจอและ endpoint ซ้ำ
...
**สิ่งที่ยังอยู่:** `sgi_consideration_logs` (ประวัติผลพิจารณารายเอกสาร — คนละเรื่องกับ audit ของ master) · `sgi_interface_transactions` (tracking รับ–ส่งไฟล์) · audit ของ RBAC/config/email template ที่อยู่ฝั่งระบบ SBP เดิม

## มติจากการเทียบฐานข้อมูลจริง (2026-08-10 · DP-1 · DP-3 · DP-9)

| มติ | ผลต่อ API |
|---|---|
| **DP-9 = แยกตัดสิน** | **ตัด `GET /decisions`** (30 → 29 เส้น · Lookup 3 → 2) — FE ใช้ `GET /common/common-code?codeType=SGI_DECISION` ของระบบเดิม · เส้น `/sgi/master/factors*` และ `/sgi/master/competitors*` **คงเดิมทั้ง 8 เส้น** |
| **DP-1 = ทางเลือก B** | `POST /sgi/workflow/instances` ต้องส่ง **`referenceId` = `sgi_compensation_documents.id` (surrogate)** ไม่ใช่ `doc_no` · `GET /sgi/document/{docNo}/timeline` และเส้นที่ต้องคุยกับ engine ต้อง **join ผ่าน `id`** ก่อนเรียก engine · `docNo` ยังเป็น path parameter ที่ผู้ใช้เห็นเหมือนเดิม |
| **DP-3 = ทางเลือกที่ 3** | ไม่กระทบ endpoint — `sgi_impacted_stores` ยังเป็นตาราง (snapshot บางส่วน) เส้นที่อ่านข้อมูลร้านไม่เปลี่ยน |

**ยังค้าง 6 ข้อ** (DP-2 · DP-6 ถึง DP-8 · DP-11 · DP-12 — **DP-4 ปิดแล้ว 2026-08-24** เมื่อตัด Job 1 ImportQSSI · **DP-10 ปิดแล้ว** SGI อยู่ใน store-backend เดิม) — ดู [`SBP/SBPGI-vs-existing-system.md`](SBP/SBPGI-vs-existing-system.md)
...
| เส้นเดิมของ SGI | ใช้ของระบบ SBP แทน |
|---|---|
...
>
> | # | function | พารามิเตอร์ (ตามชีต Detail) | SGI ใช้ที่ไหน |
> |---|---|---|---|
> | 1 | `initializeWorkflow` | `version, userId, referenceId` | เปิด flow ให้เอกสารใหม่ (Job 8b · `POST /sgi/workflow/instances`) |
> | 2 | `eventWorkflow` | `version, referenceId, event, eventParam, remark, userId` | `POST /sgi/document/{docNo}/actions` |
...
| `POST /sgi/document/{docNo}/attachments` · `GET .../download` | เขียน storage layer เอง | เก็บ metadata ใน `sgi_document_attachments` ของ SGI แต่ **ไฟล์ใช้ service S3 เดิม** `POST /statement/upload-file-aws` · `POST /statement/download-file-aws` · **[✅ DP-8 ปิดแล้ว 2026-08-24]** ใช้ตาราง `sgi_document_attachments` ของเราเอง (เก็บ metadata) + service S3 ของระบบเดิม ไม่ต่อยอด `upload_general` ของระบบเดิม (`job_id`/`audit_log_id` เป็น nullable ไม่ติด FK — เหตุผลจริงคือขาด `file_size`/`content_type`/`section_code`/`upload_status`/`purge_flag`) — [`SBP/SBPGI-vs-existing-system.md`](SBP/SBPGI-vs-existing-system.md) หัวข้อ 4 |
...
|---|---|
| **ข้อมูลยอดขายเพิ่มเติม** | K2 เดิมลิงก์ออก **QlikView BI** (`bidashboard.cpall.co.th/qlikview/FormLogin.htm`) — ระบบใหม่คงลิงก์ BI ไว้เป็นช่องทางดูเชิงลึก (URL เก็บใน config ไม่ hardcode) · `GET /sgi/document/{docNo}/sales` ใช้เป็นข้อมูลประกอบ **ไม่มีกราฟในหน้าเอกสารแล้ว (2026-08-06)** |
| **คลิกเปิดเอกสาร Statement** | ลิงก์ตรงจาก `sgi_compensation_documents.statement_id` (ฟิลด์ที่ 10 ของไฟล์ `BPM06001O_`) — ไม่ใช่ endpoint ของ SGI |
| **แผนที่ AllMap** | `sgi_compensation_documents.allmap_url` (ฟิลด์ที่ 9 ของไฟล์ `BPM06001O_`) · **iframe ล้มได้จริง** (`Failed to fetch` ในภาพหน้าจอ) → FE ต้องมี fallback + ปุ่มเปิดแท็บใหม่ |
| **คำนวณเงินชดเชย (ขั้น 08)** | **iframe ของระบบ Finance & Account Unit (FS)** — ไม่ใช่หน้าจอของ SGI · ถ้าไม่ได้ล็อกอิน FS จะได้ `401 Unauthorized` → ต้องแสดงข้อความและปุ่มเปิดแท็บใหม่แทนกรอบเปล่า |
...
| อาร์กิวเมนต์ | ความหมาย | SGI เติมค่าจากไหน |
|---|---|---|
...
| `param` | ค่าที่ lib เอาไปแทน `{{key}}` ใน subject/body | SGI สร้างจากเอกสาร เช่น `{docNo, storeName, amount, sectionName}` |
...
2. **`email_template` จริงมี 12 คอลัมน์ ไม่ใช่ 8 และชื่อไม่ตรงเอกสาร** — เอกสาร lib เสนอ `email_id`/`email_name`/`subject_mail`/`body_mail`/`mail_from`/`mail_from_name` แต่ production คือ `email_template_id` · `email_template_name` · `email_template_desc` · `subject_format` · `body_format` · `sender` · `email_from` · `active_flag` · `create_by/date` · `update_by/date` · **seed template ของ SGI ต้องใช้ชื่อจริง** และอย่าลืม `active_flag='Y'`
...
5. **ต้องยืนยันกับทีมเจ้าของ `@srm/glb-workflow`:** ถ้า engine ส่งเมลเองด้วยบน route ที่มี `email_id` ผู้อนุมัติจะได้ **เมลซ้ำ 2 ฉบับ** — ทางแก้คือใช้ `workflow_route.email_id` เป็นค่า *อ่านอย่างเดียว* แล้วให้ SGI เรียก lib เอง
...
- เทียบ SGI กับระบบ SBP เดิม + **12 ข้อค้างตัดสินใจ (Decision Points)**: `SBP/SBPGI-vs-existing-system.md` หัวข้อ 4
