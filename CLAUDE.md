...
3. **Implementation specs** for building the real system — `plan-fe.md`, `plan-be.md`, `checklist-fe.md`, `checklist-be.md`, `REACT-TODO-CHECKLIST.md` (per-screen component inventory). The FE target is a module inside the existing Next.js portal (`SBP/srm-sps-spsap-web-frontend`, portal `sbpm`) and the BE target is NestJS + TypeORM matching `SBP/srm-sps-spsap-store-backend` — decided 2026-08-05.
...
Three moving parts: 18 contract pages (+ `index.html` redirect stub and the kept-for-reference RBAC pair `k2-operators.html`/`k2-permissions.html`, which are no longer linked), one stylesheet (`assets/sbp.css`), one script (`assets/sbp.js`, ES5 IIFE, no dependencies). The header and sidebar do **not** exist in the HTML — `sbp.js` injects them at runtime on every page.
...
**Adding a page = 2 steps**: (1) create the HTML file following the contract above, (2) add an entry to the `MODULES` array in `assets/sbp.js` (~line 52) — `{key, label, href, icon, group}`. `MODULES` is the single registry driving the sidebar and breadcrumb; sidebar groups render in first-appearance order (current groups: `ระบบประกันรายได้ (SBP Mall)`, `ผู้ดูแลระบบ (Admin)` — Global Config / Batch Job / Email Template, moved there 2026-08-06, `Flow`, `Database`, `Plan` — the `(SBP Mall)` suffix signals this is the SBP Mall web front per the SDD). `email-template.html` (Email Template — 8 templates EM-01–08 with a self-contained WYSIWYG editor: toolbar, variable chips, tables, sticky save) lives in the SBP Mall group despite its `plan-` filename.
...
- `database.md` — the 24-table target schema, mirrors `plan-database.html` (data spine, zones A/B/C, cross-system keys, P0/P1 fixes).
