...
|---|---|---|---|
| **4.1** | **DP-2** — `sps_store.workflow_transaction` **ไม่มี PK และไม่มี index เลย** ทั้งที่มี 19,283 แถว (ตารางชื่อเดียวกันใน `sps_auth` มี PK ปกติ) | ทีมเจ้าของ `@srm/glb-workflow` | `GET /sgi/document/tasks` และ `POST /sgi/document/{docNo}/actions` query ตารางนี้**ทุกครั้ง** → seq-scan ทุกคำขอ |
