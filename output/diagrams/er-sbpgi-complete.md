...
| ตาราง | คอลัมน์ | PK | ความสัมพันธ์ออก | แถวจริง |
|---|---|---|---|---|
| `compensation_documents` | 25 | id | ออก 13 · เข้า 9 | — |
| `document_running_numbers` | 3 | year | ออก 1 · เข้า 0 | — |
| `document_new_stores` | 8 | id | ออก 2 · เข้า 1 | — |
...
| `consideration_logs` | 9 | id | ออก 5 · เข้า 0 | — |
...
|---|---|---|---|---|---|---|
| `sbpgi.compensation_documents.(BE service)` | N:1 | `sps_store.mas_param.param_name` | api | ค่ากำหนดกลาง — SBPGI อ่านอย่างเดียว ไม่มีหน้าจอแก้ | confirmed | database.md §ตารางที่ตัดออกรอบ 2 |
...
| `sbpgi.compensation_documents.impacted_store_code` | N:1 | `sps_store.statement.store_id` | logical | ใบแจ้งยอดของร้าน/งวด — Period Statement ของรายงาน (SDD สไลด์ 60) | proposed | database.md §โซน B |
| `sbpgi.compensation_documents.impacted_store_code` | N:1 | `sps_store.store.store_id` | logical | ชื่อร้าน · ภาค (zone_cd) · ประเภทสาขา บนหน้าเอกสาร/รายงาน | confirmed | LLDD-BE-API-Document-Detail-Aggregate.md |
...
| `sbpgi.compensation_documents.total_compensation_amount` | N:1 | `sps_store.common_code.code_value` | logical | วงเงินอนุมัติ SBPGI_APPROVE_LIMIT (เกณฑ์เดียว 100,000) | confirmed | database.md §ตารางที่ตัดออกรอบ 2 · SDD GI |
...
| `sbpgi.consideration_logs.result` | N:1 | `sps_store.common_code.code_value` | logical | ผลพิจารณา — code_type=SBPGI_DECISION (มติ DP-9) | confirmed | database.md §มติ DP-9 · การ map decisions |
...
| `sbpgi.fgi_impact_processes.impacted_store_code` | N:1 | `sps_store.fcs_monthly_sales.store_id` | logical | cross-check ยอดรวมรายเดือน (แทน sales_transactions รายวันไม่ได้) | confirmed | database.md §ผลการเทียบ ข้อ 5 |
...
| `sbpgi.interface_transactions.correlation_id` | 1:N | `sps_store.integration_log.service` | api | payload ราย call แทนตาราง FGI_WS_LOG (ข้อ F6) — ยังไม่มีคอลัมน์คีย์เชื่อมกลับ | proposed | database.md §F6 |
...
| `sps_auth.mas_store.branch_id` | 1:1 | `sps_store.store.store_id` | logical | รหัสร้านเดียวกันทั้งสองสกีมา — SBPGI ใช้ฝั่ง sps_store | confirmed | db-schema ทั้งสองไฟล์ |
...
| `sps_store.workflow_route.email_id` | N:1 | `sps_store.email_template.email_template_id` | logical | เลข template ของ workflow — SBPGI อ่านค่านี้ไปเรียก sendEmail() ของ email-lib เอง (ปิด DP-5 · 14/08/2026) | confirmed | database.md §ปิด DP-5 · db-schema-sps_store.md §workflow_route |
...
erDiagram
    ...
    sbpgi__compensation_documents }o--|| sps_store__statement : "impacted_store_code→store_id"
    sbpgi__fcs_qssi_score }o--|| sps_store__store : "store_id→store_id"
    sbpgi__fgi_impact_processes }o--|| sps_store__fcs_monthly_sales : "impacted_store_code→store_id"
    sbpgi__document_attachments }o--|| sps_store__upload_general : "object_key→key"
    sbpgi__interface_transactions ||--o{ sps_store__integration_log : "correlation_id→service"
    sbpgi__consideration_logs }o--|| sps_store__common_code : "result→code_value"
    sbpgi__compensation_documents }o--|| sps_store__common_code : "total_compensation_amount→code_value"
    sbpgi__compensation_documents }o--|| sps_store__mas_param : "(BE service)→param_name"
    sbpgi__document_running_numbers ||--o{ sbpgi__compensation_documents : "year→year"
    sbpgi__fgi_impact_competitors ||--o{ sbpgi__document_competitors : "competitor_code→source_system"
    ...
    sbpgi__compensation_documents }o--|| sps_store__store : "impacted_store_code→store_id"
```
