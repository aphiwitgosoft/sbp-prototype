...
| ผลลัพธ์ | **20 ตาราง · 24 index · 20 PK · 28 FK · 26 CHECK · generated column 2 ตัว** <br>⚠️ **ตัวเลขนี้เป็นของรอบนั้น (2026-09-13)** — ปัจจุบันคือ **27 index · 27 CHECK** (เพิ่ม index รองรับ FK 3 เส้น + CHECK `account_month` เมื่อ 2026-09-17) · ยึดยอดสดจาก `output/sql/sgi_schema.sql` เสมอ |
...
## 🔬 ตรวจ `output/sql/` รอบที่ 3 — 2026-09-16 (เทียบกับ "สิ่งที่ติดตั้งจริงบนฐาน dev")

รอบก่อน ๆ ตรวจไฟล์กับตัวเอง (generator · docker · determinism · atomicity) รอบนี้เปลี่ยนมุม:
**เอาสคริปต์ไปเทียบกับสภาพฐาน dev ที่ติดตั้งไปแล้วจริง ๆ** แล้วไล่ความสอดคล้องภายในของ seed

### ✅ ไม่มี drift เลยระหว่างสคริปต์กับฐาน

| เทียบอะไร | สคริปต์ | ฐาน dev | ผล |
|---|---:|---:|---|
| ตาราง | 20 | 20 | ✅ |
| คอลัมน์ | 251 | 251 | ✅ ชื่อครบ |
| ชนิด / NOT NULL / DEFAULT | 251 | 251 | ✅ ตรงทุกคอลัมน์ |
| PRIMARY KEY | 20 | 20 | ✅ |
| FOREIGN KEY | 28 | 28 | ✅ ตรงทุกเส้น (ตารางลูก.คอลัมน์ → ตารางแม่) |
| CHECK | 26 | 26 | ✅ |
| UNIQUE | 19 | 19 | ✅ |
| index `idx_*` | 24 | 24 | ✅ |

⚠️ รอบแรกตัว parser ของผมเองรายงาน drift 40 จุด — **เป็น false positive ทั้งหมด**
(`BIGSERIAL` = NOT NULL + `nextval` โดยปริยาย · `CHAR(n)` = `character(n)` · คอลัมน์ PK เป็น NOT NULL เอง ·
FK ที่ประกาศข้ามบรรทัด · FK ที่เพิ่มด้วย `ALTER`) — แก้ parser แล้วจึงเหลือ 0

### ✅ `sgi_existing_stub.sql` ตรงกับฐานจริง ไม่ใช่แค่ตรงกับไฟล์ dump

ที่ผ่านมา stub ถูกเทียบกับ `SBP/db-schema-sps_store.md` เท่านั้น รอบนี้เทียบกับ `information_schema` ของ dev:

```
stub    13 ตาราง · 266 คอลัมน์
ฐาน dev 13 ตาราง · 266 คอลัมน์
คอลัมน์ที่ stub มีแต่ฐานไม่มี (เทสผ่านแต่ของจริงพัง)  →  0 จุด ✅
คอลัมน์ที่ฐานมีแต่ stub ไม่มี                          →  0 จุด (stub ครบทั้งตาราง ไม่ใช่ subset)
ชนิดข้อมูลไม่ตรง                                      →  0 จุด (2 จุดที่รายงานคือ TIMESTAMPTZ = timestamp with time zone)
```

### ✅ โครงสร้าง literal ของ seed ปลอดภัยหลังใส่ HTML ภาษาไทย

เดินอ่านทีละตัวอักษรโดยเคารพ `''` · dollar-quote · คอมเมนต์:

- string literal **341 ตัว ปิดครบทุกตัว** · dollar-quote ปิดครบ
- literal ที่มี `$` **14 ตัว** (`${docNo}` ฯลฯ) · มี `;` 8 ตัว (`&nbsp;`) · ขึ้นบรรทัดใหม่ 8 ตัว
- **ไม่มี literal ตัวไหนมีรูปแบบ `$tag$`** → ไม่มีทางไปปิด dollar-quote ของ `DO $$` ก่อนเวลา ✅
- ลำดับถูก: `BEGIN` → `pg_advisory_xact_lock` → INSERT แรก → `COMMIT` ✅

⚠️ `\set ON_ERROR_STOP on` **ไม่มี `;` ปิด** (เป็น psql meta-command) — เครื่องมือที่ split ด้วย `;`
แบบไร้เดียงสาจะรวมมันกับ `BEGIN;` เป็นคำสั่งเดียว · `apply_sgi_sql.py` ตัด meta ทิ้งก่อนจึงไม่กระทบ

### 🔴 เจอของจริง — `SGI_APPROVE_LIMIT` กับ `common_code_type` ถูกลืมใน **3 ที่พร้อมกัน**

ทั้งสองถูกเพิ่มเข้า seed ทีหลัง แล้วอัปเดตไม่ครบทุกที่ที่ต้องรู้จักมัน:

| # | ที่ | อาการ |
|---|---|---|
| 1 | **คำสั่งล้างท้ายไฟล์ seed** | ไม่มี `common_code_type` เลย และ `common_code` กรองแค่ `IN ('SGI_DECISION','SGI_DOC_STATUS')` |
| 2 | **DO block ตรวจผลก่อน COMMIT** | ตรวจ 6 กลุ่มจาก 8 กลุ่มที่ seed ลง — ไม่นับ `common_code_type` และ `SGI_APPROVE_LIMIT` |
| 3 | **หัวไฟล์** | เขียน `common_code 13 แถว` ทั้งที่ไฟล์ INSERT จริง **14** |

**พิสูจน์ความเสียหายบนฐาน dev จริง** (แปลง `DELETE` เป็น `SELECT count(*)` ไม่ลบอะไร):

```
คำสั่งล้างชุดเก่า ลบได้ 32 แถว   ←  เหลือขยะ 4 แถว
คำสั่งล้างชุดใหม่ ลบได้ 36 แถว   ←  ตรงกับที่ seed ลงไว้พอดี
```

ขยะ 4 แถวคือ `common_code_type` 3 + `SGI_APPROVE_LIMIT` 1 — และเพราะ seed ใช้
`INSERT ... WHERE NOT EXISTS` **การติดตั้งใหม่หลังถอนจะได้ค่าเก่าค้างอยู่เงียบ ๆ**
ส่วนข้อ 2 ร้ายกว่า: ติดตั้งไม่ครบแล้ว **ยัง COMMIT ผ่าน** เพราะไม่มีใครนับสองกลุ่มนั้น

**แก้ที่ generator แล้วทั้ง 3 จุด** — ทุกที่คำนวณจาก `SGI_CODE_TYPES` / `SGI_APPROVE_LIMITS` ชุดเดียวกับที่ INSERT
จึงตกไม่ได้อีก · คำสั่งล้างเปลี่ยนเป็น `LIKE 'SGI\_%' ESCAPE '\'` และเพิ่มหมายเหตุลำดับ
(**ลบ `common_code` ก่อน `common_code_type` เสมอ**)

### ✅ rollback ถอนได้ครบ

`schema สร้าง 20 ตาราง · rollback DROP 20 ตาราง` — ครบทุกตัว · ไม่ซ้ำ · **เรียงย้อนลำดับ CREATE เป๊ะ** ·
มี `CASCADE` · DROP เฉพาะตาราง `sgi_` · ไม่มี `CREATE SEQUENCE` แยก (มาจาก `SERIAL` จึงหายไปพร้อมตาราง) ·
อยู่ในทรานแซกชันเดียว

### กฎใหม่กันถอยหลัง (`check_docs.py` · ทดสอบด้านลบครบทุกข้อ)

1. ตารางระบบเดิมที่ seed `INSERT` ต้องมีในคำสั่งล้างของหัวไฟล์
2. คำสั่งล้างใน seed กับใน rollback ต้องถอนของชุดเดียวกัน
3. คำสั่งล้าง `common_code` ต้องครอบคลุมทุก `code_type` ที่ seed ลง
4. DO block ต้องนับครบทั้ง 8 กลุ่มที่ seed ลง
5. ตัวเลขที่หัวไฟล์ประกาศ ต้องตรงกับจำนวน `INSERT` จริงในไฟล์

### ยืนยันหลังแก้

`sgi_seed_data.sql` รันจริงบนฐาน dev ด้วย `--trial` (ROLLBACK) **ผ่าน ตัวนับกลับสภาพเดิมครบ** ·
`sgi_existing_stub.sql` ตรงกับ generator · ชุดตรวจทั้ง 4 ตัวผ่านทุกข้อ
