...
เอกสารนี้กำหนดความต้องการของระบบประกันรายได้ SGI แบบรวม ครอบคลุมกระบวนการนำเข้าข้อมูลผลกระทบและยอดขาย การคำนวณ การสร้างเอกสาร การพิจารณาอนุมัติ การรายงาน การส่ง Statement และการติดตามผลการทำงานของระบบ
...
> SYS: ระบบต้องรวมการสร้างเอกสารและ workflow ไว้ภายใน SGI โดยใช้ DB transaction และ Workflow Engine ภายใน ห้ามสร้างไฟล์ BPM06001O/BPM06002O/BPM06003O หรือเรียก K2 StartInstance ใน runtime ใหม่
...
| --- | --- | --- | --- | --- |
| งาน & เอกสารประกันรายได้ | GET | /api/v1/sgi/document/tasks | role ที่มีสิทธิ์เมนูเอกสาร | งานรอท่านดำเนินการ - เอกสารที่ค้างอยู่ที่ section ของผู้ใช้ (หน้า Task Inbox) |
| งาน & เอกสารประกันรายได้ | GET | /api/v1/sgi/document | ตามสิทธิ์เมนู | ค้นหาเอกสารที่เกี่ยวข้อง - บังคับระบุปี และคืนเฉพาะเอกสารที่มีเลขที่แล้ว (กติกา SRS) |
| งาน & เอกสารประกันรายได้ | GET | /api/v1/sgi/document/{docNo} | ตามสิทธิ์เมนู | เอกสารฉบับเต็ม 12 ส่วนย่อย (Document Detail) พร้อมธงสิทธิ์แก้ไขต่อส่วนตาม role/section ปัจจุบัน |
| งาน & เอกสารประกันรายได้ | POST | /api/v1/sgi/document | 02 HQ, 03 User Admin | สร้างเอกสารจากข้อมูลที่ FS/SBP Statement ส่งกลับ - ตัดสินใจ 2026-08-06: ไม่มีฟอร์มสร้างเอกสารใน FE แล้ว (Create Document เหลือเป็นหน้าอธิบายกระบวนการ) เส้นนี้เรียกโดย pipeline/service token |
| งาน & เอกสารประกันรายได้ | PUT | /api/v1/sgi/document/{docNo} | ตาม section ปัจจุบัน | บันทึกแก้ไขส่วนย่อยของเอกสาร (ร้านใหม่ / คู่แข่ง / ปัจจัย) ตามสิทธิ์ของขั้นที่ถืออยู่ |
...
| ข้อมูล Lookup | GET | /api/v1/sgi/lookup/workflow-sections | ทุก role | รายการ Section 5 ขั้น + วงเงินอนุมัติต่อขั้น - dropdown ตำแหน่ง/ตัวกรอง · FE แสดงวงเงินจากข้อมูล ไม่ hardcode |
| Master Data | GET | /api/v1/sgi/master/competitors | ตาม section ปัจจุบัน | master แบรนด์ร้านคู่แข่ง 11 รายการ (รหัส 01-11 · ชื่อไทย+อังกฤษ) - dropdown ตอนกดปุ่ม "เพิ่ม" ตารางร้านคู่แข่งเปิดกระทบ (Document Detail) · จัดการที่หน้า Competitor Master |
| Master Data | POST | /api/v1/sgi/master/competitors | Admin / ผู้ดูแล master | เพิ่มแบรนด์ร้านคู่แข่งใน master (หน้า Competitor Master) - เพิ่ม 2026-08-06 ตามหน้าจอ K2 เดิม |
| Master Data | PUT | /api/v1/sgi/master/competitors/{code} | Admin / ผู้ดูแล master | แก้ไขชื่อไทย/อังกฤษ/รายละเอียดของแบรนด์คู่แข่ง - แก้รหัสไม่ได้ |
| Master Data | DELETE | /api/v1/sgi/master/competitors/{code} | Admin / ผู้ดูแล master | ลบแบรนด์คู่แข่งออกจาก master - ห้ามลบถ้ายังถูกอ้างในเอกสาร |
| Master Data | GET | /api/v1/sgi/master/factors | 03 User Admin | รายการปัจจัยภายนอก (sgi_external_factors) |
| Master Data | POST | /api/v1/sgi/master/factors | 03 User Admin | เพิ่มปัจจัยภายนอก - รหัสห้ามซ้ำ (กติกา SRS) |
| Master Data | PUT | /api/v1/sgi/master/factors/{code} | 03 User Admin | แก้ไขปัจจัยภายนอก |
| Master Data | DELETE | /api/v1/sgi/master/factors/{code} | 03 User Admin | ลบปัจจัยภายนอก (ต้องไม่ถูกใช้ในเอกสารใด) |
| รายงาน | GET | /api/v1/sgi/report/status-summary | บัญชี / 06 Report Admin | รายงานตรวจสอบประกันรายได้ (SBP Mall) - ค้นหาข้อมูล · filter 7 ตัวและผลลัพธ์ 14 คอลัมน์ ตาม SDD สไลด์ 60 · บังคับระบุปี และเอาเฉพาะเอกสารที่มีเลขที่ (กติกา SRS) |
| รายงาน | GET | /api/v1/sgi/report/status-summary/export | 04 / 06 Report Admin | Export Excel - ส่งออกผลการค้นหา 14 คอลัมน์เป็น Excel ให้ทีมบัญชีนำไปกระทบ SAP · เงื่อนไขเดียวกับเส้นค้นหา |
