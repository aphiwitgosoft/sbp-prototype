...
![รูปที่ 2: Sequence diagram: LLDD BE - Integration with SBP Platform](../../assets/flows/BE-LLDD-BE-Integration-SBP-Platform-sequence.png)

_รูปที่ 2: Sequence diagram: LLDD BE - Integration with SBP Platform_
...
| `accept-language` | `th` | BFF ส่งต่อจาก browser | เลือกภาษาข้อความ error — SGI ใช้ **ไทย verbatim ตาม SRS** เป็นค่าตั้งต้นเสมอ | ไม่บล็อก — default `th` |

⚠️ **ข้อควรระวัง `x-user-permissions` — ใช้เป็นด่านเสริมเท่านั้น:** เอกสารวิเคราะห์ระบบเดิมยืนยันแค่ว่า *มี* header ตัวนี้และเนื้อหาคือชุดสิทธิ์ต่อ URL (`canView` / `canManage` / `canExport` / `canOther`) แต่**รูปแบบ serialize ไม่ผูกเป็นสัญญา** จึงต้องเขียนโค้ดเผื่อไว้ 2 เรื่อง — (1) รูปแบบที่ serialize มา (JSON ตรง ๆ · base64 · หรือย่อเป็น CSV) และ (2) พฤติกรรมเมื่อสิทธิ์เยอะจน header ยาวเกินลิมิตของ proxy (ปกติ ~8 KB) · **จนกว่าจะยืนยัน ห้ามใช้ header นี้เป็นด่านเดียว** — ให้ตัดสินสิทธิ์เขียนจาก `x-user-group-id` + สถานะเอกสาร + ผู้ถืองานจาก `getTransaction()` ของ engine เป็นหลัก แล้วใช้ `x-user-permissions` เป็นด่านเสริม
...
🔴 **ข้อจำกัดที่ต้องรู้ก่อนเขียนโค้ด (ตรวจ `store-backend` 2026-08-26):** `AwsService` ของระบบเดิมเป็น **wrapper แบบ base64** ไม่ใช่ stream — `upload-file-aws` รับไฟล์เป็น base64 และ `download-file-aws` **คืนไฟล์เป็น base64 ใน JSON** สายส่งจริงจึงเป็น `FE ← binary stream ← SGI BE ← base64 JSON ← /statement/download-file-aws ← S3`
...
| FE ไม่ควรรู้ว่าใต้ท้องเป็น base64 | — | สัญญาฝั่ง FE ยังเป็น **binary stream + Content-Type / Content-Disposition** ตาม `LLDD-BE-API-Attachment-Sales-Timeline` — SGI เป็นคนแปลง |

⚠️ **ข้อจำกัดที่ต้องออกแบบเผื่อ:** wrapper ของระบบเดิมอาจไม่รองรับ **range request / partial download** · ถ้าไม่รองรับ ไฟล์ใหญ่จะ resume ไม่ได้ และปุ่มดาวน์โหลดทั้งหมดต้องกำหนดเพดานจำนวน/ขนาดรวม

**แก้ความเข้าใจผิดเดิม (ตรวจ schema จริง 2026-08-07):** เอกสารรุ่นก่อนเคยเขียนว่าถ้าใช้ตาราง `upload_general` ของระบบเดิมจะติด FK `job_id` — **ไม่จริง** `job_id` และ `audit_log_id` เป็น **nullable ทั้งคู่** · เหตุผลจริงที่ SGI ต้องมี `sgi_document_attachments` ของตัวเองคือ `upload_general` **ไม่มีคอลัมน์** `file_size` · `content_type` · `section_code` · `upload_status` · `purge_flag`
...
🔴 **สองตารางนี้เป็นของระบบ SBP เดิม อยู่ใน schema `sps_store` เท่านั้น** และ **ค่าของ SGI ยังไม่มีอยู่จริง — เป็นแถวที่เราต้อง seed เองตอน setup** (เอกสารรอบก่อนเขียนกำกวมจนอ่านเหมือนมีข้อมูลอยู่แล้ว · แก้ 2026-08-25)
...
| --- | --- | --- | --- |
| วงเงินอนุมัติ | **MSSQL** `SectionProfile.SectionLimitCost` — มีค่าเดียวคือ section 02 (GM) = 100,000 · AVP เป็น NULL | `sps_store.common_code` `SGI_APPROVE_LIMIT` | 🔴 **ห้าม migrate ค่าเดิมมาตรง ๆ** — เกณฑ์เก่าไม่ตรง SDD GI · **seed ใหม่** เป็นเกณฑ์เดียว 100,000 |
| ผลการพิจารณา | **MSSQL** `DecisionProfile` | `sps_store.common_code` `SGI_DECISION` | แปลงชื่อ 3 ชุด (ปุ่ม/flow/ผลลัพธ์) ลง `code_name` / `code_mapping` / `other_value` แล้ว seed |
| ปัจจัยภายนอก | **MSSQL** `FactorProfile` | **ตาราง `sgi_external_factors` ของ SGI** (ไม่ได้ไป `common_code` · มติ DP-9) | migrate เข้าตารางของเราเอง เพราะมีหน้าจอ CRUD และช่องข้อความของ `common_code` ไม่พอ |
| ร้านคู่แข่ง 11 แบรนด์ | **MSSQL** `CompetitionProfile` + **ORA** `MAS_STORE_COMPETITOR` | **ตาราง `sgi_competitors` ของ SGI** (มติ DP-9) | migrate เข้าตารางของเราเอง |
...
**สรุปสั้น ๆ:** ค้นใน `mas_param`/`common_code` แล้วไม่เจอเป็นเรื่อง**ปกติและถูกต้อง** — (1) ของเดิมอยู่คนละฐานข้อมูล (MSSQL/Oracle) · (2) ค่าของ SGI ยังไม่ถูก seed · (3) ถ้าเปิดผิด schema (`sps_auth`) จะยิ่งไม่เจอเพราะ `mas_param` ไม่มีในนั้นเลย

- 🔴 **`code_type` เป็น `varchar(20)`** (ขณะที่ `common_code_type.code_type` เป็น `varchar(50)`) — `SGI_APPROVE_LIMIT` ยาว 19 ตัว **เหลือที่ว่างแค่ 1 ตัวอักษร** · ตั้งชื่อ `code_type` ใหม่ห้ามเกิน 20
- ระบบเดิม**ไม่มี POST/PUT/DELETE ของ `common_code`** (module `common` มีแต่ GET) — SGI จะเขียนลง lookup กลางที่ทุกโมดูลใช้ร่วม ต้องทำผ่าน migration script ที่ review ได้ ไม่ใช่หน้าจอ
- SGI **อ่านอย่างเดียวในเวลาปกติ** — แก้ค่าทำที่ระบบ SBP เดิม (หน้าจอ Global Config ของ SGI ถูกลบไปแล้ว 2026-08-06) · การเขียนเกิดเฉพาะตอน **seed/cutover** เท่านั้น
...
| Progress | BFF forward request พร้อม header ตัวตนมาที่ store-backend; Guard ตรวจ x-api-key แล้ว map header เป็น user context; Controller/Service ทำงานโดยใช้ user context (ไม่มี JWT ของ SGI เอง); ผลลัพธ์ถูกห่อด้วย ResponseInterceptor เป็น {success, data} |
| Output | sgi_document_attachments (SGI) |

### 5.90 Endpoint Implementation Contract

| Endpoint | Use-case owner | Service/repository behavior | Definition of done |
| --- | --- | --- | --- |
| Internal service | กำหนดวิธีที่ SGI ต่อกับแพลตฟอร์ม SBP เดิม: BFF header/ตัวตน, response envelope, ไฟล์บน S3, อีเมลผ่าน @gosoft-sbp/email-lib และค่ากำหนดกลางใน mas_param/common_code — เป็น blocker ที่ต้องปิดในสัปดาห์แรก | เรียกจาก use case ภายในเท่านั้น | ไม่มี endpoint ใดของ SGI ออก/ตรวจ JWT เอง |
...
| 3 | Controller/Service ทำงานโดยใช้ user context (ไม่มี JWT ของ SGI เอง) | ไม่ส่ง x-user-id ต้องได้ 401 · ส่ง x-user-group-id ที่ไม่ตรง section ของเอกสาร ต้องได้ 403 |
...
| 5 | ไฟล์แนบวิ่งผ่าน service S3 เดิม · เก็บเฉพาะ metadata ใน SGI | upload ไฟล์ 6MB ต้องถูก block ก่อนขึ้น S3 · download ไฟล์ของเอกสารที่ผู้ใช้ไม่เกี่ยวข้องต้องถูก block |
...
| 7 | ค่ากำหนด/วงเงินอ่านจาก mas_param และ common_code ทุกครั้ง ไม่ cache ข้ามรอบโดยไม่มี TTL | เปลี่ยนค่า SGI_APPROVE_LIMIT ใน common_code แล้ว route อนุมัติเปลี่ยนตามโดยไม่ deploy |
...
| 3 | Controller/Service ทำงานโดยใช้ user context (ไม่มี JWT ของ SGI เอง) |
| 4 | ผลลัพธ์ถูกห่อด้วย ResponseInterceptor เป็น {success, data} |
| 5 | ไฟล์แนบวิ่งผ่าน service S3 เดิม · เก็บเฉพาะ metadata ใน SGI |
...
| 7 | เปลี่ยนค่า SGI_APPROVE_LIMIT ใน common_code แล้ว route อนุมัติเปลี่ยนตามโดยไม่ deploy |
