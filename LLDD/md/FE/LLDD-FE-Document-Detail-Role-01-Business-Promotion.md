...
| Objective | อธิบายหน้าจอ Document Detail สำหรับ role 01 - หน่วยงานส่งเสริมธุรกิจ SBP |
...
- Role profile P-01 - หน่วยงานส่งเสริมธุรกิจ SBP
...
![รูปที่ 2: Sequence diagram: LLDD FE - Document Detail Role 01 Business Promotion](../../assets/flows/FE-LLDD-FE-Document-Detail-Role-01-Business-Promotion-sequence.png)

_รูปที่ 2: Sequence diagram: LLDD FE - Document Detail Role 01 Business Promotion_
...
```json
{
  ...
  "actionOptions": [
    ...
    {
      "value": "ฝ่าย SBP DSA ดำเนินการ (ส่งกลับ)",
...
| --- | --- | --- |
| Load exact profile | เรียก GET /api/v1/sgi/document/{docNo} และยืนยัน roleProfileCode=P-01, statusCode=01 ก่อน render action state | profile mismatch ต้อง fail closed; ไม่ใช้ role switcher เพื่อจำลอง P-01 |
...
ตัวอย่าง positive-path จาก section 01; Section 02 ส่งต่อ AVP (03) เมื่อยอดรวม ≥ 100,000 บาท และจบที่ GM เมื่อ < 100,000 บาท (มติ 2026-08-18)
...
| Path ไฟล์ | หน้าที่ |
| --- | --- |
| src/app/(main)/sgi/document/[docNo]/page.tsx | route page — ใช้ร่วมกับหน้า detail — view ของ workflow section 01 |
| src/components/sgi/document-detail/RoleView01.tsx | component — view เฉพาะ workflow section 01 (อ่าน visibleSections/editableSections จาก API) |
| src/components/sgi/document-detail/ActionForm01.tsx | component — ฟอร์มผลการพิจารณา (result + comment) ของ section นี้ |
...
```tsx
'use client';
// RoleView01 — view ของหน้า Document Detail สำหรับ workflow section 01
// editableSections ตาม contract: sec-newstore, sec-competitor, sec-factor
...
//   - เห็นควรไม่ชดเชย (ต้องกรอกความคิดเห็น)
//   - ฝ่าย SBP DSA ดำเนินการ (ส่งกลับ) (ไม่บังคับความคิดเห็น)
...
export default function RoleView01({ doc, onSubmitAction, submitting }: Props) {
...
/** GET /api/v1/sgi/document/{docNo} — โหลด role profile P-01 สำหรับหน้า detail */
...
/** POST /api/v1/sgi/document/{docNo}/actions — ตัวอย่าง positive-path จาก section 01; Section 02 ส่งต่อ AVP (03) เมื่อยอดรวม ≥ 100,000 บาท และจบที่ GM เมื่อ < 100,000 บาท (มติ 2026-08-18) */
...
#### 8.6 ฟอร์มพิจารณา + validation — `src/components/sgi/document-detail/ActionForm01.tsx`
...
```tsx
'use client';
// ActionForm01 — ฟอร์ม "ผลการพิจารณา" ของ workflow section 01
...
//   - ฝ่าย SBP DSA ดำเนินการ (ส่งกลับ) (value='ฝ่าย SBP DSA ดำเนินการ (ส่งกลับ)', requireComment=false)
// editableSections ของ role นี้ (ใช้เป็น constant สำหรับ assertion/test เท่านั้น ไม่ใช่เพื่อ hardcode การ render):
export const EDITABLE_SECTIONS_01 = ['sec-newstore', 'sec-competitor', 'sec-factor'] as const;
...
export default function ActionForm01({ options, onSubmit, onCancel, submitting }: {
...
| --- | --- | --- |
| `roleProfileCode` | validation | ผ่านเมื่อถูกกฎ / โยน error เมื่อผิด — กฎ: must match API response · รูปแบบ: P-01 |
| `statusCode` | validation | ผ่านเมื่อถูกกฎ / โยน error เมื่อผิด — กฎ: from API · รูปแบบ: 01 |
