# LLDD FE - Document Detail Role 02 GM Business Promotion
...
| Objective | อธิบายหน้าจอ Document Detail สำหรับ role 02 - GM ส่งเสริมธุรกิจฯ |
...
| --- | --- | --- | --- |
| roleProfileCode | P-02 | must match API response | ใช้เลือก view profile เฉพาะบทบาทนี้; แยก namespace จาก workflow section code |
| statusCode | 02 | from API | workflow status/section code ปัจจุบัน ไม่ใช่ role profile |
...
| --- | --- |
| Role profile | P-02 - GM ส่งเสริมธุรกิจฯ |
| Workflow section/status code | 02 |
| Document status shown | รอ GM ส่งเสริมธุรกิจ SBP ดำเนินการ |
| Purpose on this page | อ่านข้อมูลประกอบการอนุมัติวงเงินและส่งผลพิจารณา |
| Editable sections | - |
...
| No | Test |
| --- | --- |
| 1 | เปิดด้วย roleProfileCode=P-02 แล้วทุก business section ต้อง read-only |
| 2 | sec-calc ต้องไม่ render |
| 3 | action radio แสดงเฉพาะ 3 รายการของ role 02 |
| 4 | แสดง totalCompensationAmount/approvalLimitIndicator โดยไม่คำนวณ route ใน FE |
...
| --- | --- | --- |
| Load exact profile | เรียก GET /api/v1/documents/{docNo} และยืนยัน roleProfileCode=P-02, statusCode=02 ก่อน render action state | profile mismatch ต้อง fail closed; ไม่ใช้ role switcher เพื่อจำลอง P-02 |
...
| Path ไฟล์ | หน้าที่ |
| --- | --- |
| src/app/(main)/sbpgi/documents/[docNo]/page.tsx | route page — ใช้ร่วมกับหน้า detail — view ของ workflow section 02 |
...
```tsx
'use client';
// RoleView02 — view ของหน้า Document Detail สำหรับ workflow section 02
...
export default function RoleView02({ doc, onSubmitAction, submitting }: Props) {
...
/** GET /api/v1/documents/{docNo} — โหลด role profile P-02 สำหรับหน้า detail */
...
/** POST /api/v1/documents/{docNo}/actions — ตัวอย่าง positive-path จาก section 02; Section 02 ส่งต่อ AVP (03) เมื่อยอดรวม ≥ 100,000 บาท และจบที่ GM เมื่อ < 100,000 บาท (มติ 2026-08-18) */
...
```tsx
'use client';
// ActionForm02 — ฟอร์ม "ผลการพิจารณา" ของ workflow section 02
...
export default function ActionForm02({ options, onSubmit, onCancel, submitting }: {
...
| --- | --- | --- |
| `roleProfileCode` | validation | ผ่านเมื่อถูกกฎ / โยน error เมื่อผิด — กฎ: must match API response · รูปแบบ: P-02 |
| `statusCode` | validation | ผ่านเมื่อถูกกฎ / โยน error เมื่อผิด — กฎ: from API · รูปแบบ: 02 |
