...
![รูปที่ 2: Sequence diagram: LLDD FE - Document Detail Role 06 SBP DSA](../../assets/flows/FE-LLDD-FE-Document-Detail-Role-06-SBP-DSA-sequence.png)

_รูปที่ 2: Sequence diagram: LLDD FE - Document Detail Role 06 SBP DSA_
...
| --- | --- | --- |
| Load exact profile | เรียก GET /api/v1/sgi/document/{docNo} และยืนยัน roleProfileCode=P-06, statusCode=06 ก่อน render action state | profile mismatch ต้อง fail closed; ไม่ใช้ role switcher เพื่อจำลอง P-06 |
...
ตัวอย่าง positive-path จาก section 06; Section 02 ส่งต่อ AVP (03) เมื่อยอดรวม ≥ 100,000 บาท และจบที่ GM เมื่อ < 100,000 บาท (มติ 2026-08-18)
...
| Path ไฟล์ | หน้าที่ |
| --- | --- |
| src/app/(main)/sgi/document/[docNo]/page.tsx | route page — ใช้ร่วมกับหน้า detail — view ของ workflow section 06 |
| src/components/sgi/document-detail/RoleView06.tsx | component — view เฉพาะ workflow section 06 (อ่าน visibleSections/editableSections จาก API) |
| src/components/sgi/document-detail/ActionForm06.tsx | component — ฟอร์มผลการพิจารณา (result + comment) ของ section นี้ |
...
```tsx
'use client';
// RoleView06 — view ของหน้า Document Detail สำหรับ workflow section 06
...
export default function RoleView06({ doc, onSubmitAction, submitting }: Props) {
...
/** GET /api/v1/sgi/document/{docNo} — โหลด role profile P-06 สำหรับหน้า detail */
...
/** POST /api/v1/sgi/document/{docNo}/actions — ตัวอย่าง positive-path จาก section 06; Section 02 ส่งต่อ AVP (03) เมื่อยอดรวม ≥ 100,000 บาท และจบที่ GM เมื่อ < 100,000 บาท (มติ 2026-08-18) */
...
```tsx
'use client';
// ActionForm06 — ฟอร์ม "ผลการพิจารณา" ของ workflow section 06
...
export default function ActionForm06({ options, onSubmit, onCancel, submitting }: {
...
| --- | --- | --- |
| `roleProfileCode` | validation | ผ่านเมื่อถูกกฎ / โยน error เมื่อผิด — กฎ: must match API response · รูปแบบ: P-06 |
| `statusCode` | validation | ผ่านเมื่อถูกกฎ / โยน error เมื่อผิด — กฎ: from API · รูปแบบ: 06 |
