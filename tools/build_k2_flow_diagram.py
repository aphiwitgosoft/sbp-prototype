...
def diagram_swimlane() -> str:
    ...
    b += box(620, cy1, 250, 60, VIOLET, "ประเภทเคส ?  (ระบบตัดสิน)",
             ["① เปิดเรื่องใหม่ · ② ต่อเนื่อง (แตกต่อด้วยยอดชดเชย)"], tint="#f5f3ff")
    ...
    # ② ต่อเนื่อง — ตัดสินใจชั้นที่ 2 ด้วยยอดชดเชย แล้วรวมเป็นเส้นเดียวเข้า 08
    b += path(f"M745,{cy1} L1240,{cy1} L1240,{cy3-41}", VIOLET, "arv")      # ② (ใช่) → 08
    b += tag(760, cy1-28, "② ต่อเนื่อง · ยอดชดเชย > 0 หรือ ยอด 0 ติดกัน ≤ 3 เดือน", VIOLET)
    b += tag(760, cy1-13, "→ ระบบ Auto Approve → เข้า 08 (ข้ามขั้น 06)", VIOLET)
    # ② (ไม่ใช่) · ยอด 0 ติดกัน > 3 เดือน → เส้นแดงเข้ารางจบที่ x=1900 (ไม่เข้า flow พิจารณา)
    b += path(f"M620,{cy1-30} L620,{cy1-56} L1900,{cy1-56} L1900,{cy2-78}", RED, None, dash="7 5")
    b += tag(1085, cy1-64, "② ต่อเนื่อง · ยอดชดเชย 0 ติดกัน > 3 เดือน → หยุดชดเชยประกันรายได้ · จบทันที",
             RED, "middle", 11.5)   # baseline เหนือเส้นแนวนอนที่ cy1-56
    ...
    b += path(f"M1800,{cy6+38} L1800,{cy7-30}", GREEN, "arn")
    b += tag(1812, cy6+64, "AVP เห็นควรชดเชย → จบ", GREEN, "start")
    # มติ 2026-09-02 — AVP เห็นควรไม่ชดเชย จบ flow ทันทีเหมือน 01/02 (เดิมตีกลับ 06)
    b += path(f"M1745,{cy6+38} L1745,{cy7-30}", RED, "arr", dash="7 5")
    b += tag(1733, cy6+64, "AVP เห็นควรไม่ชดเชย → จบทันที (มติ 2026-09-02)", RED, "end")
    ...
    # มติ 2026-09-01 — AVP ส่งกลับ 06 (เดิม 02) · มติ 2026-09-02 — "เห็นควรไม่ชดเชย" แยกออกไปเป็นเส้นแดงจบ flow แล้ว
    b += path(f"M1540,{cy6+26} L1370,{cy6+26} L1370,{cy6+56} L{RL},{cy6+56} L{RL},{cy4+32}",
              AMBER, None, dash="7 5")
    b += tag(1000, cy6+48, "AVP ส่งกลับฝ่าย SBP DSA → กลับ 06", "#b45309", "middle")
    ...
    return svg(W, H, b, "Flow K2 — Workflow อนุมัติประกันรายได้ (SGI)",
               "5 ขั้น 06→08→06→01→02→03 · 6 สถานะ · 2 ประเภทเคส · 3 จุดเข้า · เกณฑ์วงเงินเดียว 100,000 · ส่งกลับทุกขั้น → 06 (มติ 2026-09-01)")
...
def diagram_gate() -> str:
    W, H = 1760, 890
    b = ""
    b += pill(170, 150, 210, 62, GREEN, ["เริ่ม · เอกสารเข้าระบบ"])
    # ── ชั้นที่ 1 · ประเภทเคส — มีแค่ 2 ทาง (เปิดใหม่ / ต่อเนื่อง) ──
    b += diamond(450, 150, 132, 66, ["ประเภทเคส ?", "จากข้อมูลรอบชดเชยของร้าน"], VIOLET)
    b += path("M282,150 L312,150")

    # ① เปิดเรื่องใหม่ → 06 (ออกด้านขวาของ D0 ตรง ๆ)
    b += path("M582,150 L740,150", MUTE, "arg")
    b += tag(596, 136, "① เปิดเรื่องใหม่", MUTE)
    b += box(1060, 150, 600, 92, MUTE,
             "last_compensate_seq_no = 1   (รอบใหม่ · last_compensate_seq +1)",
             ["จาก sgi_fgi_impact_processes (hub รอบชดเชยของร้าน · โซน A)",
              "— ไม่มี auto · เดินตาม flow เดิม"], rx=10, tint="#ffffff")
    b += path("M1360,150 L1400,150", MUTE, "arg")
    b += box(1570, 150, 300, 74, BLUE, "ฝ่าย SBP DSA", ["รอฝ่าย SBP DSA ดำเนินการ"],
             badge="06", rx=10)

    # ② ต่อเนื่อง → ลงไปตัดสินใจชั้นที่ 2
    b += path("M450,216 L450,368", VIOLET, "arv")
    b += tag(462, 264, "② ต่อเนื่อง", VIOLET)
    b += tag(462, 280, "last_compensate_seq_no > 1  AND  flag_action = 'Y'", VIOLET, size=10.5)

    # ── ชั้นที่ 2 · เงื่อนไขยอดชดเชยของเคสต่อเนื่อง ──
    b += diamond(450, 434, 150, 66,
                 ["ยอดชดเชย > 0  หรือ", "ยอด 0 ติดกัน ≤ 3 เดือน ?"], AMBER)
    b += tag(450, 372, "ยอดที่ใช้ = COALESCE(adjust_amount, forecast_amount)", "#b45309",
             "middle", 10.5)

    # ②-ใช่ → Auto Approve → 08
    b += path("M600,434 L660,434 L660,380 L740,380", VIOLET, "arv")
    b += tag(614, 424, "ใช่", VIOLET)
    b += box(1060, 380, 600, 92, VIOLET,
             "ยอด > 0   หรือ   ยอด = 0 ติดกันงวดที่ 1–3",
             ["จาก sgi_fgi_impact_processes + sgi_fgi_impact_compensations (โซน A)",
              "ระบบ Auto Approve การเปิดเรื่อง — ข้ามขั้น 06 (มติ 2026-09-01 · เดิมยอด 0 เข้า 01)"],
             rx=10, tint="#ffffff")
    b += path("M1360,380 L1400,380", VIOLET, "arv")
    b += box(1570, 380, 300, 74, BLUE, "เจ้าหน้าที่ SBP DSA",
             ["รอเจ้าหน้าที่ SBP DSA ดำเนินการ"], badge="08", rx=10)

    # ②-ไม่ใช่ → เส้นแดง → หยุดชดเชย จบทันที
    b += path("M450,500 L450,600 L740,600", RED, "arr")
    b += tag(462, 560, "ไม่ใช่ · ยอด 0 ติดกัน > 3 เดือน", RED)
    b += box(1060, 600, 600, 92, RED,
             "ยอด = 0 ติดกันงวดที่ 4 ขึ้นไป",
             ["จาก sgi_fgi_impact_compensations (ยอดชดเชยรายงวด · โซน A)",
              "Auto หยุดชดเชยประกันรายได้ — ปิดเอกสารทันที"], rx=10, tint="#ffffff")
    b += path("M1360,600 L1400,600", RED, "arr")
    b += pill(1570, 600, 300, 66, RED,
              ["จบทันที · ไม่เข้า flow พิจารณา", "เสร็จสิ้นดำเนินการ (หยุดชดเชยฯ)"])
    ...
    return svg(W, H, b, "จุดเข้า flow ตามประเภทเคส (To-Be)",
               "ผัง To-Be 12/02/2026 · ประเภทเคสมี 2 ทาง — ① เปิดเรื่องใหม่ เริ่มที่ 06 · ② ต่อเนื่อง ตัดสินต่อด้วยยอดชดเชย (เข้า 08 หรือ หยุดชดเชยฯ)")
...
def diagram_states() -> str:
    ...
    b += txt(78, 408, "• เตือนงานค้างทุกวันจันทร์ 10:00 น. (Job 12) · escalation เมื่ออายุงานอยู่ใน \"ช่วง\" 30-36 / 45-51 / 60-66 วัน — ไม่ใช่ \"ครบ n วันขึ้นไป\" นอกช่วงไม่ส่ง", 11.5, "#475569", "start")
...
DIAGRAMS = [
    ...
    ("02-entry-gate", 1760, 700, "จุดเข้า flow ตามประเภทเคส",
     "ประเภทเคสมี 2 ทาง — เปิดเรื่องใหม่ / ต่อเนื่อง · เคสต่อเนื่องตัดสินต่อด้วยยอดชดเชย (เข้า 08 หรือ หยุดชดเชยฯ)"),
