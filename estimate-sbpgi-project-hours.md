...
> ### ✅ มติ 2026-08-21 — ชั่วโมงงาน ใช้ชุด LLDD ชุดเดียว
>
> **ตรวจแล้วสองชุดนี้ไม่ได้ขัดกันเรื่องขอบเขต — ขัดกันที่หน่วย "ชั่วโมงต่อวัน" เท่านั้น**
>
> | ชุด | ชั่วโมง | ชม./วัน | = man-day |
> |---|---|---|---|
> | ไฟล์นี้ (FE 510 + BE 595 + Shared 102) | 1,207 | 8.5 | **142.0** |
> | ชุด LLDD (implementation 675 + unit test 154) | **829** | 6.0 *(สมมติฐานเก่าของชุดนี้)* | **138.2** |
> | | | | ต่างกัน **3.8 md · 2.7%** |
>
> ⚠️ **แก้เลขคำนวณ 2026-09-08:** เดิมเขียน "ต่างกัน 8.7 md · 6.1%" ซึ่งเป็นค่าตกค้างจากตอนที่ชุด LLDD ยังเป็น **800 ชม.**
> (800 ÷ 6 = 133.3 → ต่าง 8.7 md = 6.1%) · ยอดปัจจุบัน **829 ชม.** ให้ 829 ÷ 6 = **138.2** → ต่าง **3.8 md = 2.7%**
>
> **หมายเหตุสำคัญ:** ตาราง 2 บรรทัดบนเป็น **การเทียบสมมติฐานเก่า** ที่ใช้ตัดสินมติ 2026-08-21 เท่านั้น —
> ตั้งแต่มติ 2026-08-25 ทั้งระบบใช้ **8.5 ชม./วัน หน่วยเดียว** ดังนั้นเทียบที่หน่วยเดียวกันจะได้
> **829 ÷ 8.5 = 97.5 md** vs **1,207 ÷ 8.5 = 142.0 md** — ต่างกัน **44.5 md (31.3%)** ซึ่งคือขนาดของขอบเขตที่ตัดออกไปจริง
>
> **มติ: ใช้ชุด LLDD `829 ชั่วโมง` @ **8.5 ชม./วัน (42.5 ชม./สัปดาห์ · 5 วัน)** — กรอบ 2026-08-25 “จบใน 4 สัปดาห์” = **170 ชม./คน** (823 → 845 หลังรับ F8+F1 และงานจุดเข้า flow 2026-08-21 → 824 หลังตัด Job 1 ImportQSSI 2026-08-24 → 800 หลังปรับตามของที่ reuse ได้จาก `srm-sps-spsap-sop-sgi-batch` → **829** หลังเพิ่ม Job 11/12 ปิดช่องว่าง 2.8/2.9 · 2026-09-02) เป็นตัวเลขอ้างอิงเดียวของโครงการ** เพราะ (1) แตกละเอียดถึงระดับเอกสารรายฉบับ (2) รวม unit test แล้ว (3) `worklist.html` และหน้าแผนงานรายสัปดาห์ใช้ **42.5 ชม./สัปดาห์** ชุดเดียวกับ LLDD
>
> ⚠️ **แก้ 2026-09-08:** ค่าชุดนี้เคยเขียนเป็นค่าก่อนมติ 2026-08-25 (วันละ 8 ชม. · สัปดาห์ละ 40 และ 30) — ค่าที่ใช้จริงทั้งระบบคือ **8.5 / 42.5** แหล่งเดียวคือ `HOURS_PER_DAY` / `HOURS_PER_WEEK` ใน `tools/build_lldd_documents.py`
>
> ไฟล์นี้เก็บไว้เป็น **baseline เดิม (superseded)** — ใช้อ้างอิงย้อนหลังได้ แต่**ห้ามนำตัวเลขไปใช้คู่กับชุด LLDD ในเอกสารเดียวกัน**

> **ปรับขอบเขต 2026-08-06:** หน้า **ตั้งค่าระบบ (Global Config)** และ **Email Template** ถูกยกเลิกทั้งฟีเจอร์ (ใช้ `mas_param` / `email_template` ของระบบ SBP เดิม) และหน้า **Batch Job** ย้ายไปกลุ่มเมนู Flow เหลือเฉพาะ Flowchart + Database ที่ใช้ (ตัด 2 tab ควบคุมออก) — งานในตารางด้านล่างที่เป็นของ 3 หน้านี้ (FE และ BE) **ตัดออกจากขอบเขต** ส่วนตัว batch job ยังทำและรันตามปกติโดยกำหนดค่าใน backend config
