...
export const API_GROUPS: ApiGroup[] = [
  ...
  },
  {
    ...
    eps: [
      { m: 'GET', p: `${B}/tasks`, ref: 'k2', refT: 'K2 · 3.1.2', sum: 'inbox งานค้างที่ section ของผู้ใช้',
        ...
        sql: 'SELECT t.doc_no, d.status_code, s.store_name\nFROM workflow_tasks t\nJOIN compensation_documents d ON d.doc_no = t.doc_no\nJOIN impacted_stores s ON s.store_code = d.impacted_store_code\nWHERE t.section_code = :section AND t.status = :open;' },
      ...
      { m: 'POST', p: `${B}/documents/{docNo}/actions`, ref: 'k2', refT: 'K2 · 3.1.4', roles: 'เจ้าของ task', sum: 'ส่งผลพิจารณา — workflow 7 ขั้น · กฎ 100,000 (มี Flowchart)',
        ...
        sql: 'UPDATE workflow_tasks SET status = :closed WHERE doc_no = :docNo AND status = :open;\nINSERT INTO consideration_logs (doc_no, section_code, result) VALUES (:docNo, :cur, :action);\nUPDATE compensation_documents SET status_code = :next WHERE doc_no = :docNo;' },
  ...
  },
  {
    ...
    eps: [
      ...
      { m: 'GET', p: `${B}/workflow-sections`, ref: 'k2', refT: 'K2', sum: 'รายการ Section 7 ขั้น' },
    ],
  },
  {
    name: '4 · Master Data',
