...
กลุ่ม sidebar render ตามลำดับที่พบครั้งแรก ปัจจุบัน **4 กลุ่ม**: `ระบบประกันรายได้ (SBP Mall)` → `Flow` → `Database` → `Plan` (**กลุ่ม `ผู้ดูแลระบบ (Admin)` ไม่มีอยู่แล้ว** — ลบทั้งกลุ่มเมื่อ 2026-08-06)
...
| | `k2-competitors.html` | **master แบรนด์คู่แข่ง 11 รายการ (รหัส 01–11 · ไทย+อังกฤษ)** — เพิ่ม 2026-08-06 · อยู่ถัดจากกำหนดปัจจัยภายนอก · ป้อน dropdown "ร้านคู่แข่ง (Master)" ในหน้าเอกสาร · **อย่าสับสนกับ `sgi_document_competitors`** ที่เก็บคู่แข่งรายสาขาที่ import จาก ALLMAP |
...
| | | **กลุ่ม `ผู้ดูแลระบบ (Admin)` ถูกลบทั้งกลุ่ม** — ไม่ใช่แค่ไม่มีเมนู แต่ไม่มีชื่อกลุ่มนี้ใน `MODULES` แล้ว |
| Flow | `flow-fgi.html` / `k2-flow.html` / `plan-flow.html` / `new-flow-improved.html` | FGI/FCS pipeline / K2 approval BPMN / flow รวมระบบใหม่ (คู่ของ workflow.md) |
...
| Database | `fgi-database.html` / `k2-database.html` / `plan-database.html` / **`database-dictionary.html`** / `output/diagrams/er-sgi-complete.html` | schema FGI/FCS / schema K2 16 ตาราง + ER / schema รวม **20 ตาราง** (19 CREATE + `fcs_qssi_score` ที่ reuse · คู่ของ database.md · RBAC/ผู้ปฏิบัติงาน + workflow engine + store/zone/employee master + email template + config ตัดไปใช้ของระบบ SBP เดิม · 2026-08-05/06) |
| Plan | `worklist.html` | **หน้าจัดการงานสไตล์ Notion** (งาน → API → DB) + Kanban `#/board` + แผนงานรายสัปดาห์ `#/plan` — เพิ่ม 2026-08-18 · **generate ด้วย `tools/build_worklist.py` ห้ามแก้ไฟล์ตรง** · เป็นหน้า standalone (มี sidebar ของตัวเอง ไม่ใช้ page contract ของ `sbp.js`) จึงมีลิงก์ `← กลับระบบประกันรายได้` ในตัว |
| Plan | `plan-api.html` | REST API spec **28 เส้น 6 กลุ่ม** (Lookup 2 · Master Data 8 · เอกสาร 11 · รายงาน 2 · Workflow 3 · Interface 2 · กลุ่ม Auth + เส้นสิทธิ์/ผู้ปฏิบัติงาน 18 เส้น comment ตัดถาวร — ใช้ระบบเดิม · กลุ่มข้อมูลผิดปกติ 2 เส้น · กลุ่ม System Config + Email Template 10 เส้น · กลุ่ม Batch Job Admin 6 เส้น — ลบทิ้งทั้งหมด 2026-08-06) |
...
- catalog `GROUPS` (**28 เส้น 6 กลุ่ม** · inline script) → คลิกแถว → `selectEp()` เปิด modal
...
- **`FLOWCHART_BY_PATH`** — spec flowchart (nodes/edges) เฉพาะเส้นซับซ้อน **3 เส้น** (ชื่อ path ปัจจุบันหลังมติ namespace 2026-08-25 + เปลี่ยนชื่อเป็น `sgi` 2026-09-01): `POST /api/v1/sgi/document/{docNo}/actions` · `POST /api/v1/sgi/workflow/instances` · `POST /api/v1/sgi/document` (spec ของ `POST /jobs/{jobNo}/run` **ถูกลบออกแล้ว** พร้อมกลุ่ม Batch Job Admin — เหลือ 3 เส้น · ตรวจยืนยัน 2026-08-24) · เรนเดอร์ด้วย `renderFlow()` เป็น inline SVG (node type: term/termOk/proc/dec/err)
...
**`database-dictionary.html` (เพิ่ม 2026-09-08)** — พจนานุกรมข้อมูล **19 ตารางใหม่ (โซน A/B/C) · 223 คอลัมน์** ตอบ 4 คำถามต่อตาราง (มาจากไหน · ใช้ทำอะไร · ทำไมต้องมี · ทุกคอลัมน์เก็บอะไรไว้ใช้ทำอะไร) · **generate จาก `tools/lldd_db_dictionary.py` ห้ามแก้ไฟล์ HTML ด้วยมือ** (สร้างใหม่ทุกครั้งที่รัน `build_lldd_documents.py`) · คู่กับเอกสาร `LLDD-Database-Dictionary` ใน `LLDD/md/`
