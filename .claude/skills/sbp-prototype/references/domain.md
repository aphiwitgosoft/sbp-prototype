...
- เลขที่เอกสาร `YYYY/xxxxx` — **ปี ค.ศ.** เช่น `2026/00123` (running เริ่ม 00001) · **มติ 2026-08-06: วันที่และเลขเอกสารเป็น ค.ศ. ทั้งระบบ** (ยกเว้นไฟล์ interface ของ **STA** คือ `FRBC0001_*` ที่เป็น พ.ศ. + windows-874 แปลงตอนอ่าน/เขียนไฟล์เท่านั้น · ⚠️ **แก้ 2026-09-16 จากไฟล์ตัวอย่างจริง** — ไฟล์ของ IAS `AMS06001O_*`/`AMS06001I_*` เป็น **ค.ศ. ไม่ใช่ พ.ศ.** และไม่มีไบต์ภาษาไทยเลย · รายละเอียดทั้งหมดใน `docs/IAS-STA-interface-files.md`) · DDL บังคับไว้แล้วที่ `sgi_document_running_numbers.year` = "ปี ค.ศ. เท่านั้น ห้ามเก็บ พ.ศ."
