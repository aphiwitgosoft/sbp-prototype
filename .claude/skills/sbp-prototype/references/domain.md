...
- Schema 34 ตาราง 3 โซน (A=FGI/FCS · B=K2 docs/workflow · C=shared master/config · RBAC/ผู้ปฏิบัติงานใช้ระบบ SBP เดิม) + Data Spine 4 ID (`impact_process_id` → `doc_no` → `instance_id` → `task_id`) → `database.md`
