...
Frontend (Web SPA — ใช้หน้าจอ prototype ชุดนี้เป็น spec)
        │  REST API /api/v1 · JSON + JWT · 39 เส้น (ดู plan-api.html)
        ▼
Backend Services
...
7. **ขั้น 1–2: ฝ่าย SBP DSA ตรวจสอบ + คำนวณเงินชดเชย** (K2 · Section 06 → 08) — ผู้ใช้ทำงานบน FE (หน้างานรอดำเนินการ / เอกสารร้านถูกกระทบ) · พิจารณา: ชดเชย / ไม่ชดเชย / หยุดชดเชย · บันทึก `consideration_logs` + อีเมลตาม `status_email_rules`
8. **ขั้น 3: ฝ่ายส่งเสริมธุรกิจฯ ปรับข้อมูล** (K2 · Section 01) — แก้ไขร้านเปิดใหม่ / ร้านคู่แข่ง / ปัจจัยภายนอก (**%ชดเชยรวมต้อง = 100%**) แล้วส่งพิจารณาต่อ หรือส่งกลับ (back-flow)
...
3. **สร้างเอกสารแบบ manual** — ขั้น 15 ของ flow เดิม (กรอกข้อมูลร้านเอง เมื่อระบบคำนวณให้ไม่ได้) → ระบบใหม่: หน้า "สร้างเอกสาร" (k2-create) → `POST /documents` ลงตารางชุดเดียวกับเส้นอัตโนมัติ — จุดเข้าเสริม ไม่แตะ batch
...
- API ทุกเส้น: `plan-api.html` (39 endpoints — กลุ่มข้อมูลผิดปกติ 2 เส้นถูก comment รอตัดสินใจ)
