...
- [ ] `formatDate('2026-01-15')` → `15/01/2026` (**ค.ศ.**) · `formatMoney(1234567)` → `1,234,567.00`
...
- [ ] สร้างเอกสารนอกเงื่อนไข → ได้เลข `2026/xxxxx` (ปี ค.ศ.) → เอกสารสถานะ `รอฝ่าย SBP DSA ดำเนินการ`
...
- [ ] `ReportSearchForm.tsx` (**7 ตัวกรองตาม SDD สไลด์ 60** · 2026-08-06): สถานะ\* (select 6 ค่า · **Required Field ตัวเดียว**) | รหัสร้านถูกกระทบ (numeric) · รหัสร้านเปิดกระทบ (numeric) | **ประเภทร้าน (checkbox `A/B/C/E`)** · **Period Statement From–To** (`input[type=date]` ปฏิทิน วัน/เดือน/ปี **ค.ศ.** เต็มแถว · บังคับเมื่อสถานะ = เสร็จสิ้นดำเนินการ) · ภาค (`REPORT_REGIONS13` — checkbox เต็มแถว · ภาคใหม่แสดงอัตโนมัติ) · **radio ผลการพิจารณา (ไม่บังคับ): `ประกันรายได้` / `ไม่ประกันรายได้`** เต็มแถว · ปุ่ม `เคลียร์ค่าเริ่มใหม่` / **`ค้นหาข้อมูล`** / `Export Excel` · **validate:** ระบุรหัสร้านถูกกระทบแล้วต้องระบุรหัสร้านเปิดกระทบด้วย
- [ ] คอนโทรลทุกชนิดในฟอร์มสูงเท่ากัน (46px) — ใช้ token กลาง ไม่ตั้งความสูงรายหน้า
- [ ] ตารางผล 14 คอลัมน์ **sort ได้ทุกคอลัมน์** (text / number / date) คลิกหัวคอลัมน์สลับ asc–desc
- [ ] ไม่เลือกปี → error + ไม่ยิง API (กฎเดียวกับ related)
- [ ] `ค้นหาข้อมูล` → `GET /reports/status-summary` → `SummaryLine` (พบ N รายการ / ยอดชดเชยรวม / วงเงินเข้า AVP / แถวแดง) · **ไม่มีกราฟในหน้ารายงานแล้ว (ถอด 2026-08-06)**
- [ ] `ReportResultTable.tsx` **14 คอลัมน์ (SDD สไลด์ 60)** (scroll ใน `.table-wrap`): `รหัสร้านถูกกระทบ | ชื่อร้านถูกกระทบ | ภาค | ประเภทร้าน | เดือน/ปีที่ถูกกระทบ | Period Statement | รหัสร้านเปิดกระทบ | ชื่อร้านเปิดกระทบ | ภาค | ประเภทร้าน | ยอดเงินชดเชย | ครั้งที่ | วันที่สร้าง | เลขที่เอกสาร` · `flag-red` · ประเภทร้าน/ภาคเป็นรหัสสั้น · วันที่เป็น ค.ศ.
- [ ] ปุ่ม **`Export Excel`** → `GET /reports/status-summary/export` (เงื่อนไขเดียวกับการค้นหา) → ดาวน์โหลดไฟล์ `.xlsx`
...
- [ ] ติ๊กภาค 2 รหัส + ประเภท A → ผลถูกกรองตรงกัน · ตาราง 14 คอลัมน์ scroll แนวนอนโดย body ไม่ scroll
- [ ] Export Excel → เปิดไฟล์ `.xlsx` ภาษาไทยไม่เพี้ยน เนื้อหา 14 คอลัมน์ตรงกับผลการค้นหา
...
- `GM_LIMIT = 50_000` / `AVP_LIMIT = 300_000` (SDD GI 24/02/2026 — แทน `AVP_THRESHOLD` 100_000 เดิม) — ใช้แค่**แสดงป้าย** (แถบสรุปหน้ารายงาน, แผง S10) — routing จริงอยู่ BE
